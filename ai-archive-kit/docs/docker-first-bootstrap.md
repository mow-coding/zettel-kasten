...
AI Archive Kit uses a Docker-first hybrid setup for non-programmer users.

The user's computer can be Windows, macOS, or Linux. The Archive Kit runtime runs inside a Linux container so the CLI, MCP server, SQLite behavior, and future server pieces behave the same way across the team.

## User-Facing Flow

```text
1. Run the AI Archive Kit setup script.
