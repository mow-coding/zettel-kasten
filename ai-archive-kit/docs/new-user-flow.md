...
This document explains the normal safe workflow for a beginner using AI Archive Kit.
...
The beginner path is Docker-first. That means Windows and Mac users do not need to set up Python directly. Run the setup dry-run from inside `ai-archive-kit/`; it checks Docker and tells you exactly what will happen.
...
```powershell
python ai-archive-kit\cli\archive.py validate ai-archive-kit\examples\fake-life-archive --format json
```
...
```powershell
python ai-archive-kit\cli\archive.py pilot-plan `
...
```powershell
python ai-archive-kit\cli\archive.py preflight .\archives\personal-life --strict --check-docker
```

Run a restore drill before the first real source scan:

```powershell
python ai-archive-kit\cli\archive.py recovery-plan .\archives\personal-life --format json
python ai-archive-kit\cli\archive.py restore-drill .\archives\personal-life --target C:\tmp\personal-life-restore --dry-run --format json
python ai-archive-kit\cli\archive.py restore-drill .\archives\personal-life --target C:\tmp\personal-life-restore --approve --reviewed-by person:me
```

Then preflight can require a successful restore receipt:

```powershell
python ai-archive-kit\cli\archive.py preflight .\archives\personal-life --strict --require-restore-drill
```

For separation between personal and team archives:

```powershell
python ai-archive-kit\cli\archive.py preflight .\archives\personal-life --peer-archive .\archives\team-archive --strict
```
...
```powershell
python ai-archive-kit\cli\archive.py onboard `
...
```powershell
python ai-archive-kit\cli\archive.py sources .\tmp-my-archive --format json
```
...
```powershell
python ai-archive-kit\cli\archive.py source-mounts .\tmp-my-archive
```
...
```powershell
python ai-archive-kit\cli\archive.py search ai-archive-kit\examples\fake-life-archive "onboarding"
```
...
```powershell
python ai-archive-kit\cli\archive.py import-external .\tmp-my-archive `
  --source notion `
  --export .\notion-export `
  --dry-run `
  --format json
...
```powershell
python ai-archive-kit\cli\archive.py share .\tmp-fake-life-archive `
...
```powershell
python ai-archive-kit\cli\archive.py transfer-ownership .\tmp-fake-life-archive `
