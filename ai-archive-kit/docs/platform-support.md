...
AI Archive Kit is Docker-first hybrid for non-programmer users.
...
Host-native developer fallback from inside `ai-archive-kit/`:
...
```bash
PYTHONPATH=src python -m ai_archive_kit.archive_cli doctor examples/fake-life-archive --strict
```
