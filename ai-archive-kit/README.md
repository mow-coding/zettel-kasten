...
create-draft
  Create a draft zettel in inbox/. With --dry-run, preview a profile-aware inbox draft zet without writing files.
...
One-command setup from inside `ai-archive-kit/`:
...
```powershell
python ai-archive-kit\cli\archive.py runtime-context ai-archive-kit\examples\fake-life-archive --format json
```
...
```text
src/ai_archive_kit/mcp_server.py
```

Run from inside `ai-archive-kit/` without installing:
