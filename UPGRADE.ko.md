...
아카이브 마이그레이션은 필요하지 않습니다. 이 릴리스는 공개 연동 안내와 집중
회귀 검사만 추가합니다. 기존 WOM 명령, 스키마, zet, 오브제, 영수증, 목록,
인덱스, 쓰기 권한은 그대로입니다.

WOM은 MOW Harness 없이도 사용할 수 있습니다. 별도 하네스를 사용한다면 변경
전에 [선택적 MOW Harness 호환 경계](wom-kit/docs/mow-harness-compatibility.md)를
읽으세요. WOM은 이미 `collab/`과 `.mow-harness/`를 로컬 전용으로 유지하지만,
외부 도구를 설치·업데이트·활성화·제거하거나 대신 실행하지 않습니다.

외부 `status`, `doctor`, `--dry-run`은 검사로 다루세요. `--yes`는 별도의
하네스 쓰기 승인이고, 파일 설치나 업데이트 성공은 활성화가 아닙니다. 기존
`OFF` 방은 사람이 활성화를 명시적으로 요청하고 하네스 자체 근거가 남기 전까지
계속 꺼 둡니다.

안정된 파일 기반 하네스는 여전히 `v0.1.23`입니다. 외부 CLI는 아직 소스
알파이며 이번 체크포인트에서 WOM의 npm 의존성이 아닙니다.
