...
않습니다. WOM은 원래 MOW Harness를 번들하거나 호출하지 않았습니다. 이번
릴리스는 과거의 외부 추천, 사용할 수 없는 저장소 링크, 전용 호환 안내,
설치·업데이트·활성화 설명을 제거합니다.

저장소 전용 아티팩트 위생 검사는 중립적인 소스 코드 별칭
`LOCAL_ONLY_COORDINATION_STATE`를 제공하되, 기존의 정확한 기계 출력·import
호환 이름 `LOCAL_ONLY_COLLAB_HARNESS`를 그대로 유지합니다. 기존 parser와
import는 바꿀 필요가 없습니다. WOM CLI 명령, MCP 도구, 스키마, zet, 오브제,
영수증, 목록, 인덱스, provider 계약은 바뀌지 않습니다.

이름이 남아 있다는 이유만으로 `collab/`이나 `.mow-harness/`를 지우면 안
됩니다. 사람이 쓴 계획, 프롬프트, mailbox, 설치 메타데이터, 비밀정보가 있을 수
있습니다. 기본 아카이브 루트 Doctor 검사, 아카이브 루트 source 탐색,
restore drill, 저장소 아티팩트 위생 검사는 두 경로를 제외하고, 아티팩트 검사기는
두 경로 자체를 직접 대상으로 받아도 거부합니다. 이 격리가 사람이 명시적으로
고른 경로까지 안전하게 만들지는 않으므로 별도의 파일 캡처나 staged-folder
명령에 두 경로를 지정하면 안 됩니다. 정확한
비아카이브 설치 경로를 백업·검토한 운영자는 낡은 외부 도구 바이트를 별도로
정리할 수 있지만, WOM은 자동 삭제 권한을 부여하지 않으며 이번 종료 처리로
개인 아카이브를 수정하지 않습니다.

자세한 내용은
[`wom-kit/docs/releases/v0.3.306.md`](wom-kit/docs/releases/v0.3.306.md)와
[`wom-kit/docs/archive-infra-decision-log-2026-08-08-v03306-mow-harness-sunset.md`](wom-kit/docs/archive-infra-decision-log-2026-08-08-v03306-mow-harness-sunset.md)를
보세요.
...
하나와 그 파일의 정확한 SHA-256을 준비한 뒤 다음 명령을 실행합니다.
...
이 명령은 전체 제목 증거 감사가 완전하고 깨끗한지, 현재 정본이 영수증의
적용 후 상태와 같은지, 이전 바이트 snapshot과 manifest가 유효한지, 원래
변경이 정확히 제목 하나만 바꾼 작업이었는지를 다시 검증합니다. 파일을
쓰거나 지우지 않습니다.

snapshot을 정본 위에 손으로 복사하지 마세요. v0.3.273은 사람이 검토할
계획 digest만 만들며 실제 승인형 되돌리기는 아직 구현하지 않습니다.
원본 영수증과 snapshot을 그대로 보존하세요. 자세한 내용은
[`wom-kit/docs/zet-title-remap-revert-plan.md`](wom-kit/docs/zet-title-remap-revert-plan.md)를
참고하세요.
...
뒤, 남아 있는 제목 리맵 transaction journal에는 새 읽기 전용 계획을
사용하세요.
...
계획은 완전한 잔류 사건마다 한 가지 고정 판단을 내립니다. 시작되지 않은
거래 증거 정리, 검증된 이전 바이트로 미커밋 부분·전체 적용 원복, 정확히
검증된 완료 잔여 증거 정리, 또는 `manual_forensic_hold`입니다. 원본 감사나
반환 사건 집합이 불완전하면 계획 자체를 차단합니다.

이 명령은 아무것도 쓰거나 지우지 않습니다. action은 사람이 검토할 판단이지
보관함을 편집할 권한이 아닙니다. 모든 영수증·journal·공통 lock·이전 바이트
snapshot을 보존하세요. v0.3.271은 복구를 실행하지 않고 완료된 제목 영수증을
되돌리지 않습니다. 이 계획을 검토할 때는 v0.3.271 이상을 사용하세요.
자세한 내용은
[`wom-kit/docs/zet-title-remap-recovery-plan.md`](wom-kit/docs/zet-title-remap-recovery-plan.md)를
보세요.
...
없습니다. 읽기 전용 명령 하나가 추가됩니다.

```powershell
archive zet-abstract-backfill-recovery-plan <archive-root> --dry-run --format json
```

이 명령은 v0.3.265 트랜잭션 저널과 기존의 제한된 영수증/잠금 감사를 읽고,
증거 정리, apply 되돌리기, revert 계속 진행/영수증 마무리, 수동 포렌식 보류 중
하나를 권고합니다. 파일을 쓰거나 지우지 않습니다.

출력은 결정 문서이지 사람이 직접 파일을 고쳐도 된다는 허가가 아닙니다. 모든
case는 실제 복구 실행기가 아직 구현되지 않았으며, 나중에 새 복구 승인과 실행
직전 상태 재검증이 필요하다고 명시합니다. 중단된 저널과 잠금은 계속 보존하세요.

저널이 없는 기존 아카이브에서 실행해도 `no_recovery_needed`를 반환합니다.
v0.3.266은 새 아카이브 형식을 만들지 않으므로, 이전 WOM-kit은 단지 이 새 명령을
제공하지 않을 뿐입니다.

## v0.3.265 초록 일괄 작업 내구성 저널

기존 아카이브, zet, 인덱스, 영수증을 마이그레이션할 필요가 없습니다. 새 명령이나
승인 옵션도 없습니다. 다만 아래 두 기존 승인 작업의 내부 순서가 달라집니다.
...
이제 두 명령 모두 첫 정본 zet를 고치기 전에
`.wom-scratch/abstract-backfill/` 아래에 비공개 트랜잭션 저널을 씁니다. 저널은
정확한 참여 zet 아이디/경로와 변경 전후 해시, 제안서 또는 원본 영수증 해시, 최종
영수증 경로, 검토 권한을 기록합니다. 본문이나 초록 텍스트는 기록하지 않습니다.

apply 잠금과 저널은 공용 `.wom-scratch/abstract-backfill/` 루트에서 제안서 SHA로
이름을 정합니다. 그래서 같은 제안서 바이트를 서로 다른 파일명이나 하위 폴더에
복사해도 반드시 같은 잠금에서 충돌합니다.

정상 완료하면 불변 영수증을 쓰고 검증한 뒤 저널을 지웁니다. 프로세스 안에서 오류가
나도 모든 참여 파일이 원래 바이트로 돌아왔음을 검증한 뒤에만 저널을 지웁니다.
프로세스가 강제 종료되거나 되돌리기가 불완전하면 저널과 기존 잠금을 그대로
남깁니다.

남은 증거는 기존 읽기 전용 명령으로 확인합니다.

```powershell
archive zet-abstract-backfill-receipt-audit <archive-root> --dry-run --format json
```

검진 결과는 작업 시작 전, 일부만 반영, 전부 반영됐지만 영수증 없음, 외부 변경으로
불일치, 정상 완료 뒤 정리 파일만 남음 상태를 구분합니다. 비공개 아이디·경로·검수자,
저널 해시, 본문, 초록은 출력하지 않습니다.

검진을 초록색으로 만들겠다고 남은 저널이나 잠금을 지우면 안 됩니다. 이번 릴리스는
중단 상태를 **진단할 뿐**, 자동으로 이어 쓰거나 끝내거나 빠진 영수증을 만들거나
되돌리지 않습니다. 다음 복구 릴리스 또는 신중한 포렌식 판단 때까지 증거를
보존하세요.

새 영수증은 기존 `crash_recovery_journal_written` 필드를 `true`로 기록하고,
v0.3.265는 이 값이 `false`인 과거 영수증도 계속 검증합니다. 하지만 v0.3.265로
승인된 초록 일괄 작업을 실행한 뒤의 검진과 되돌리기는 반드시 v0.3.265 이상을
사용해야 합니다. v0.3.264 이하는 v0.1 영수증 스키마에서 `false`만 허용하므로 새
`true` 영수증을 무효로 거부하고, 그 배치를 검진하거나 되돌릴 수 없습니다.
아카이브 마이그레이션은 없지만 도구 버전에는 단방향 게이트가 생깁니다.

윈도우 한계는 v0.3.264와 같습니다. 파일 생성/교체는 원자적이지만 디렉터리 항목을
`fsync`할 수 없어서, 프로세스 강제 종료는 다루되 갑작스러운 전원 손실까지
내구성이 있다고 주장하지 않습니다.

## v0.3.264 정본 쓰기 내구성

아카이브, zet, 인덱스, 영수증, schema 마이그레이션이 필요하지 않습니다. 명령의
출력 형태와 성공·실패 결과가 그대로이고, **파일 내용도 하나도 바뀌지 않습니다.**
다만 기계적으로 두 가지가 달라집니다. 부모 폴더가 없는 곳에 쓰면 이제 실패하지
않고 폴더를 만듭니다. 그리고 목적지가 잠겨 있으면 짧게 재시도한 뒤 같은 오류를
냅니다. 또한 원자적으로 교체된 파일은 새 inode라서, zet에 걸린 하드링크는
유지되지 않고 권한은 기본값으로 새로 잡힙니다.

이번 릴리스는 오직 **쓰다가 중단됐을 때** 무슨 일이 벌어지는가에 대한 것입니다.
지금까지 정본 zet를 고치는 모든 경로가, 임시 파일을 만들어 옮기되 **데이터를
디스크에 확실히 밀어넣지 않은 채** 옮기는 도우미 함수를 거쳤습니다. 그래서 전원이
나가면 멀쩡하던 zet 자리에 빈 파일이나 잘린 파일이 남을 수 있었습니다. 같은 zet를 동시에 두 번 쓰면
서로 부딪혀 실패하기도 했습니다. 다만 원인은 처음 보이던 것과 달랐습니다 —
부딪히는 건 임시 파일이 아니라 **목적지**입니다. 윈도우는 누군가 열어둔 파일을
대체하지 않기 때문입니다. 이제 이름 바꾸기를 짧게 재시도한 뒤 포기합니다. 이건
더 흔한 경우 — 백신이나 검색 인덱서가 그 zet를 읽고 있어서 **혼자 쓰는데도**
실패하던 경우 — 까지 함께 막아줍니다. 둘 다 고쳤습니다.

**새로 쓰실 명령이나 옵션은 없습니다.** 쓰이는 바이트도 이전과 완전히 동일하며,
그걸 확인하는 회귀 테스트를 따로 두었습니다.

정리용 참고 하나: 쓰다가 중단되면 zet 옆에 숨김 임시 파일이 남을 수 있습니다.
이름은 `.<이름>.<무작위>.tmp` 형태입니다. 이번 릴리스가 그걸 쓸어 담지는
않습니다 — 정본 zet 옆의 파일을 이름 패턴만 보고 지우는 건 이 변경이 감당할
결정보다 큽니다. 모든 쓰기 경로는 정상·오류 양쪽에서 자기 임시 파일을
정리합니다.

알아두실 한계 두 가지가 있습니다. 이름 바꾼 뒤 디렉터리 항목까지 밀어넣는 것은
리눅스와 macOS에서만 됩니다. 윈도우는 디렉터리를 그 용도로 열 수 없어서, 쓰기가
성공한 직후 전원이 나가면 이전 내용이 남을 수 있습니다(다만 **잘린 파일은
어떤 경우에도 남지 않습니다**). 그리고 이제 쓰기 한 번마다 디스크 플러시가 한 번
더 듭니다. zet 수천 개를 훑는 일괄 마이그레이션에서는 체감됩니다(로컬 SSD 기준
zet당 약 1~2밀리초(하드웨어에 따라 다름), 네트워크 저장소면 더 큽니다). zet를 잃지 않는 값이고,
모아서가 아니라 쓰기마다 치릅니다.

## v0.3.263 검토형 제목 재매핑 계획

아카이브, zet, 인덱스, 영수증, schema 마이그레이션이 필요하지 않고, 기존 명령의
동작도 그대로입니다.
...
v0.3.262는 제목 자리에 가져오기 식별자가 들어간 정본 zet이 몇 건인지 알려주는
명령이었습니다. 이번 것은 **이미 가지고 계신 대체 이름을 검사합니다.**
`.wom-scratch/title-remap/` 아래에 비공개 JSONL 파일을 두시면 됩니다. zet 하나에
한 줄이고, 함께 배포되는 `zet-title-remap-proposal.schema.json` 계약에 맞춰
작성합니다. **다섯 필드가 모두 필요하며 `schema` 표시가 빠진 줄은 거부됩니다.**

```json
{"schema":"wom-kit/zet-title-remap-proposal/v0.1","zettel_id":"zet_import_notion_a1b2c3d4e5f60718","expected_file_sha256":"sha256:<64자리 hex>","title":"2026학년도 창업동아리 테스트 결과지","basis":"source_export_property"}
```

`basis`는 `source_export_property`(원본 export 속성에서 가져옴) 또는
`human_written`(사람이 직접 씀) 둘 중 하나이고, `expected_file_sha256`은 그 zet을
마지막으로 확인했을 때의 해시입니다.

아무것도 쓰지 않습니다. 어느 줄이 사람 검토를 받을 준비가 됐고 어느 줄이 막혔고
왜 막혔는지만 보고합니다. 다만 해시를 계산해야 해서 **지목된 정본 zet을 본문까지
통째로 읽습니다** — frontmatter만 읽던 v0.3.262 전수 조사보다 넓은 읽기입니다.

미리 알아두실 거절 세 가지가 있습니다.

- 지금 제목이 **사람이 지은 이름**인 zet은 거절합니다. 이 명령은 가져오기
  식별자를 고치는 도구이지 제목 편집기가 아닙니다.
- 대체 제목이 **그 자체로 식별자 모양**이면 거절합니다.
- 대체 제목이 발행 점검의 "충분히 구체적인 제목" 규칙을 통과 못 하면
  거절합니다.

제안 파일을 만든 뒤에 그 zet이 바뀌었다면 그 줄도 막힙니다. 확인하지 않은 내용을
대상으로 계획이 세워지는 일은 없습니다. 대체 제목이 `zet-title-readiness`에 다시
걸릴 것이면 — 그 기록 자신의 가져오기 식별자와 같은 경우를 포함해 — 같은 이유로
거부합니다.

준비 상태 확인 명령들과 달리, **막힌 줄이 하나라도 있으면 종료 코드 1입니다.**
이건 전부 통과하거나 아니거나인 계획이기 때문입니다. 제안 파일이 비었거나,
`--max-items`가 허용하는 줄 수보다 많거나(**기본값 500**이라 수천 줄 매핑은 값을
올리거나 나눠야 합니다), `--max-items` 자체가 범위를 벗어나도 종료 코드 1입니다.

승인 후 실제 쓰기, 영수증, 되돌리기는 이후 릴리스로 나옵니다.

## v0.3.262 식별자 제목 전수 조사
...
정본 zet 중에서 제목이 사람이 읽는 이름이 아니라 가져오기 때 딸려온 페이지
식별자인 경우가 몇 건인지 알려줍니다. 가져오기 배치가 원본 기록의 제목을
그대로 복사하면 이런 일이 생기는데, 지금까지는 아무것도 이걸 잡아내지
못했습니다. 스키마는 제목을 아무 문자열이나 허용하고, 긴 16진수 값은 발행
점검의 "충분히 구체적인 제목" 검사도 통과하기 때문입니다.

이 명령은 정본 zet의 frontmatter만 읽습니다. 제목과 가져오기 식별자 facets만
보고, 초록이나 본문은 읽지 않습니다. 아무것도 쓰지 않으며, 제목이나 식별자
값을 절대 출력하지 않습니다.

zet의 id와 파일명은 제목에서 만들어지기 때문에, 검토 목록의 행이 자기 `path`나
`zettel_id`로 문제의 제목을 그대로 재현할 수 있습니다. 그런 값은 가리고, 몇 건을
가렸는지 함께 보고합니다. frontmatter를 읽지 못했거나 제목이 비어 있거나 안전
문제로 가려진 zet은 별도로 세며, 이 경우 아카이브를 "준비됨"으로 판정하지
않습니다.

`--dry-run`이 필요하고 없으면 종료 코드 1로 막습니다. 검토가 필요하다는 결과가
나와도 명령 자체는 성공(0)입니다.

이 릴리스는 고치지는 않습니다. 검토를 거친 제목 일괄 수정은 승인 후 쓰기
방식의 별도 흐름이며, 이후 릴리스들로 나누어 나갑니다.
...
이름, 인자, 출력 형태, 종료 코드도 그대로입니다.

이번 릴리스는 이 저장소의 첫 지속적 통합(CI) 워크플로를 추가하고, 그 CI가
바로 찾아낸 크로스플랫폼 결함 2건을 고칩니다. 패키지 리소스 매니페스트를 다시
생성했지만 91개 항목과 각 항목의 바이트 수·SHA-256 값은 완전히 같고 순서만
...
포크를 유지한다면, `sync_package_resources.py`가 이제 아카이브 상대 POSIX
문자열로 정렬한다는 점을 알아두세요. 매니페스트를 한 번 다시 생성하면 만든
기계에 따라 결과가 달라지는 문제가 사라집니다.
...
텍스트, 인수인계 영수증, 인덱스와 기본 `read-zettel` 전체 본문 응답은 계속
호환됩니다.

큰 zet 본문은 선택적으로 JSON 페이지로 나누어 읽을 수 있습니다. 첫 페이지는
`--body-max-chars`와 cursor 0으로 읽습니다. 이후 0이 아닌
`--body-cursor`를 사용할 때에는 첫 페이지가 돌려준 전체 본문 해시를 반드시
`--expected-body-sha256`으로 다시 전달해야 합니다. 중간에 본문이 바뀌면 서로
다른 두 버전을 합치지 않고 멈춥니다. CLI 페이지 읽기는 `--format json`을
요구하며, 페이지 옵션을 쓰지 않으면 예전처럼 전체 본문을 반환합니다.

`derive-text coverage`와 검진은 인코딩 변환으로 바이트가 달라진 전사본이나
내보내기 원본이 오브제 목록에 없다고 새로 알릴 수 있습니다. 이것은 읽기 전용
보존 진단이지 자동 마이그레이션이 아닙니다. 실제 원본 바이트가 남아 있고
사람이 검토했을 때만 그 파일을 오브제로 담으세요. 정규화된 파생 텍스트에서
원본을 다시 만들어서는 안 됩니다. 오브제 아이디가 목록에 있다는 사실만으로
현재 로컬이나 원격에 바이트가 존재한다고 증명되지는 않습니다.

AI 산출물 목록과 새 세션 인수인계 영수증은 문서에 적힌 허용 폴더만 검사했다는
점을 명시합니다. 인수인계 준비 완료는 그 제한된 범위와 사람의 대화 검토가
준비됐다는 뜻이며, 아카이브 다른 위치에 AI 산출물이 전혀 없다는 뜻은 아닙니다.
기존의 유효한 인수인계 영수증은 계속 인식됩니다.

현재 공개 읽기 안내는 문서 순회에 `zet`를 쓰고, `node`는 주체/archive
참여자에 사용합니다. 스키마, 명령, 출력 키, 영수증, 과거 릴리스 노트와 기존
아카이브 아이디는 이름을 바꾸지 않습니다.

## v0.3.253 선택적 MOW Harness 호환 경계

이 절은 v0.3.253 당시의 문서 전용 경계를 기록한 과거 자료입니다. 외부 설치,
업데이트, 활성화 안내는 v0.3.306에서 폐기되었으므로 현재 제품 안내로 사용하면
안 됩니다. 아카이브 마이그레이션은 과거에도 지금도 필요하지 않습니다.
`collab/`과 레거시 `.mow-harness/` 경로는 옛 프롬프트, 설치 메타데이터,
비밀정보가 WOM 기록이나 공개 Git 표면에 들어가지 않게 막는 방어적 로컬 상태
격리 규칙으로만 남습니다.

## v0.3.252 설계 철학 구현 추적성

아카이브 마이그레이션은 필요하지 않습니다. 이 릴리스는 문서와 회귀 검사만
바꿉니다. 기존 명령 동작, 스키마, zet, 오브제, 영수증, 목록, 인덱스, 쓰기
권한은 그대로입니다.

WOM 설계 철학이 구현되었는지 검토할 때
[한국어](wom-kit/docs/philosophy-implementation-evidence.ko.md)와
[영문](wom-kit/docs/philosophy-implementation-evidence.md) 근거표를 사용하세요.
각 행의 설계 원칙, 구현된 표면, 회귀 근거, 정직한 경계를 함께 읽어야 합니다.

구조적 카탈로그 통과를 초록의 의미 승인으로, 영수증을 객관적 진실로, 로컬
백업 근거를 현재 원격 상태 증명으로 해석하면 안 됩니다. 이 부분은 실사용
또는 외부 서비스별 검증 경계로 남습니다.

## v0.3.251 정직한 로컬 백업 근거 상태

보관함 마이그레이션은 필요하지 않습니다. 이 릴리스는 읽기 전용 JSON 상태
명령 하나를 추가하며 기존 오브제 목록이나 영수증을 바꾸지 않습니다.
...
세 백업 영역을 따로 읽으세요. GitHub와 외부 DB에는 아직 외부 서비스가
확인한 범용 완료 영수증 계약이 없으므로 완료 상태를 확인하지 않았다고
보고합니다. 오브제 저장소는 유효한 `wom_uploaded` 위치가 일치하는 실행
영수증에 연결된 경우만 근거로 셉니다.

`receipt_verified_full_coverage_at_recorded_time`은 제한 안에서 끝까지 읽은
오브제 목록의 모든 대상에 영수증 기록 시점의 로컬 근거가 있다는 뜻입니다.
지금 원격 바이트를 다시 확인했다는 뜻도, WOM 전체 백업이 끝났다는 뜻도
아닙니다. `declared_uploaded`, 로컬 커밋, 설정, 생성 인덱스만으로는 충분한
근거가 되지 않습니다.

이 명령은 외부 서비스나 네트워크를 호출하지 않고, 오브제 바이트나 zet
본문을 읽지 않으며, 아무것도 쓰지 않습니다. 잘못되거나 서로 맞지 않는
근거는 영수증이나 오브제 목록을 손으로 고치지 말고 해당 오브제 저장소
절차를 통해 해결하세요.
...
바이트가 없는 해시 기반 이력으로 계속 유효합니다. 과거에 보존하지 않은
바이트를 나중에 만들어내지는 않습니다.

이제 새 일반 수정 승인 때마다 WOM-kit은 정본 zet를 바꾸기 전에 정확한 이전
바이트를 Git에서 제외된 `objects/sha256/`에 보존하고, 로컬 오브제 목록에
등록하고, v0.2 수정 영수증으로 그 오브제를 묶습니다. 처리된 실패로 정본을
되돌려도 검증된 보존본은 남고, 쓰기 뒤 중단된 영수증을 마무리할 때도 그
보존본을 먼저 검증합니다.

업데이트 뒤 다음 읽기 전용 검진을 실행하세요.

```powershell
archive zet-revision-receipt-audit <archive-root> --dry-run --format json
```

새 v0.2 영수증의 이전-상태 바이트나 목록 기록이 없거나 달라졌으면 검진이
차단됩니다. 로컬 검진 통과가 원격 백업 완료를 뜻하지는 않습니다.
`objects/sha256/`에는 기존 오브제 저장소 업로드·검증 절차를 계속 사용하고,
Git 추적을 켜지 마세요.

## v0.3.247 AI 운영자 아티팩트 우선 원칙

아카이브 마이그레이션은 필요하지 않습니다. 저장된 zet, 오브제, 엣지,
목록, 영수증, 스키마, 명령 권한은 바뀌지 않습니다.

설치된 `wom-archive` Agent Skill과 새 아카이브의 `AGENTS.md`가 아티팩트
우선과 인간 변화 원칙을 AI 운영자에게 직접 알려줘야 할 때 업그레이드하세요.
정본은 사람이 검토한 현재 아카이브 상태이지 객관적 진실이 아니며, 같은
이름이나 라벨은 동일성 병합 권한이 아니고, 생성된 연결 구조는 읽기 보조
수단이며, 모순과 달라진 의미는 출처와 함께 남겨야 한다는 규칙이 포함됩니다.

이미 관리형 runtime skill을 설치했다면 기존 `runtime-skill-install --dry-run`
뒤 정확히 같은 계획을 승인하는 업데이트 절차를 사용하세요. wheel 설치만으로
AI 호스트 설정이나 아카이브가 바뀌지는 않습니다.

## v0.3.246 아티팩트 우선과 인간 변화

아카이브 마이그레이션은 필요하지 않습니다. 실행 명령, 스키마, 저장된 zet,
엣지, 매니페스트, 영수증, 생성 색인은 바뀌지 않습니다.

이번 릴리스는 제품의 권위 관계를 명확히 합니다. 오래 남는 로컬
아티팩트와 시간순 이력이 일차 근거이며, 엔티티·그래프 투영은 다시 만들 수
있는 읽기 보조물입니다. 정본은 객관적 진실이 아니라 사용자가 승인한 현재
아카이브 상태이고, 같은 라벨은 조용한 동일성 병합을 승인하지 않습니다.

기존 아카이브는 v0.3.246을 도입하기 위해 기록을 다시 쓰거나 합치거나
정규화해서는 안 됩니다. package/release 신원과 공개 AI 운영 안내를 이 설계
경계에 맞추고 싶을 때 업그레이드하면 됩니다.
...
묶인 스킬 자료를 찾을 수 있도록 AI 운영자 프로세스를 다시 시작하세요.

내장 `wom-archive` 스킬은 이제 Agent Skills 묶음 형식을 따릅니다. YAML
`name`과 `description`이 있는 짧은 `SKILL.md` 하나, 업무별 집중 참고문서
다섯 개, 정확한 고급 명령을 위한 기존 전체 운영 계약 하나로 구성됩니다.
AI 운영자는 첫 문서를 먼저 읽고 사람의 현재 목표에 맞는 참고문서만 열어야
합니다.

릴리스 게이트는 이제 `wom-kit/tools/check_runtime_skill.py`를 실행합니다.
첫 문서 누락이나 과도한 길이, 깨졌거나 바깥으로 나가는 Markdown 링크,
첫 문서에서 찾을 수 없는 참고문서, 심볼릭 링크, 이름 불일치, 필수
승인·비밀정보·신뢰 경계 누락을 차단합니다. 설치용 wheel도 같은 파일을
바이트 단위로 묶습니다.

이번 업데이트는 Codex, Claude 같은 AI 호스트의 설정 폴더에 아무것도
설치하지 않습니다. 생성된 그래프를 만들거나 WOM의 권한 기준도 바꾸지
않습니다. 사람이 검토한 로컬 zet, objet, 관계, 영수증 기록이 계속
정본입니다.
...
`abstract-freshness`는 모든 정본 zet의 제한된 초록 데이터만 먼저 읽고,
현재 유효한 명시적 초록 데이터가 있어 본문 해시가 필요한 대상만 전체 zet
바이트를 엽니다. 선택된 대상은 전체 바이트에서 초록 데이터와 본문을 다시
해석한 뒤에만 두 해시의 짝을 신뢰합니다. 초록 데이터가 없거나, 비공개
처리됐거나, 초록 데이터를 읽을 수 없는 zet는 불필요하게 본문을 열지 않고
기존처럼 검토 항목 또는 제외 항목으로 남습니다.

결과의 `scan`에는 `canonical_frontmatter_files_scanned`,
`canonical_body_files_read`, `canonical_body_files_not_read`,
`canonical_body_read_policy`, `canonical_scan_mode`,
`canonical_scan_workers`가 추가됩니다. 기존 최신성 상태, 증거 조회, 종료 동작,
비공개 보호, 쓰기 없음 계약은 바뀌지 않습니다.

대형 옛 보관함에서는 같은 읽기 전용 최신성 명령을 다시 실행해 새 본문 읽기
수와 실행 시간을 비교합니다. 본문을 건너뛴 것을 본문 검증 완료로 해석하면 안
됩니다. 유효한 명시적 초록 데이터가 없는 zet는 이미 검토 항목이며, 더 넓은
파일 상태 검증은 별도의 검진/검증 명령이 담당합니다.
...
이력 검진이 보이도록 AI 운영자 프로세스를 다시 시작합니다.

먼저 `zet-revision-receipt-audit --dry-run`을 실행합니다. 신뢰할 수 있는 비공개
백업에서 과거 전체 zet를 `.wom-scratch/revisions/restores/` 아래에 복구하고,
`zet-revision-restore-plan`을 실행한 뒤 현재 zet, 복구한 zet, 선택한 영수증을
비공개로 함께 검토합니다. 다음으로 계획 결과의 현재 파일 해시, 복구 파일과
의미 해시, 계획 해시를 그대로 넣어 `zet-revision-restore-write --dry-run`을
실행합니다. 승인할 때는 미리보기의 사건 시간과 쓰기 계획 해시를 그대로
사용하고, 안전한 검토자 아이디와 복원 및 초록 데이터/본문 짝 검토 확인을
제공해야 합니다. 엣지가 바뀌면 엣지 검토 확인도 따로 필요합니다.

승인 후 쓰기는 일반 수정 쓰기와 같은 정본별 잠금을 사용하고 복구 파일의
바이트를 그대로 설치합니다. 과거 `updated_at`, YAML, BOM, 줄바꿈, 초록 데이터
순서, 본문을 다시 쓰지 않습니다. 새 복원 사건 시간은 불변 복원 영수증에
남습니다. 실행 중 오류가 나면 직전 바이트를 정확히 되돌리고, 프로세스가
중단됐다면 잠금을 지우거나 파일을 손으로 복사하지 말고 같은 승인 명령을
다시 실행합니다.

일반 수정 영수증과 복원 영수증은 이제 하나의 시간순 사건 이력을 이룹니다.
완료 후 영수증 검진을 다시 실행합니다. 정상 결과는 사람이 검토한 로컬 적용
근거이지, 사실성·백업 완전성·외부 동기화 증명은 아닙니다.
...
보관함 마이그레이션은 필요하지 않습니다. 새 CLI 전용 읽기 명령이 보이도록
AI 운영자 프로세스를 다시 시작합니다. 정본 zet를 한 번 이상 수정한 뒤,
다음 AI 세션에 보관함을 넘기기 전 다음 명령을 실행합니다.

```text
archive zet-revision-receipt-audit <archive-root> --dry-run --max-receipts 5000 --max-locks 5000 --max-problems 100 --progress --format json
```

정상 결과는 남아 있는 수정 영수증이 현재 zet까지 하나의 연속된 해시 이력을
이루며, 인식된 작업 잠금 중 복구가 필요한 것이 없다는 뜻입니다. 완료된 뒤
남은 잠금은 경고이고, 영수증 누락·쓰기 전 중단·판단 불가·손상·지원하지 않는
잠금은 사람이 확인해야 합니다. 검진을 통과시키려고 잠금을 지우거나 불변
영수증을 고치면 안 됩니다.

이 명령은 읽기 전용이며 결과에 zet 내용을 내보내지 않습니다. 수정 내용이
사실인지 증명하지도 않고, 영수증의 해시만으로 과거 zet를 되살릴 수도
없습니다. 따라서 되돌리기는 비공개로 복구한 전체 zet 바이트부터 사람이
검토하는 별도의 미래 절차로 남습니다.
...
운영자 프로세스를 다시 시작한 뒤 다음 읽기 전용 검진을 실행합니다.

```text
archive abstract-freshness <archive-root> --dry-run --progress --format json
```

`first-read-readiness` 다음, 비공개 전체 카탈로그를 읽기 전에 실행합니다.
`fresh`는 현재 초록 데이터와 본문의 해시 짝이 사람이 검토한 근거와 정확히
같다는 뜻입니다. `stale`은 본문, 초록 데이터 또는 둘 다 바뀌었다는 뜻이고,
`unverified`는 검토 근거를 다시 확인할 수 없다는 뜻입니다. `missing`과
`unreadable`은 구조적 공백이며, 비공개 처리된 zet는 정책상 제외됩니다.

새로 승인된 mint와 이전 `promote` 영수증은 문장을 복사하지 않고 검토 근거를
남깁니다. v0.3.232 mint 초안 스냅샷이나 남아 있는 promote 원본, 기존의 사람
검토 완료 초록 보충 영수증도 인식합니다. 기존 zet와 영수증은 그대로
유효하며 자동으로 다시 쓰지 않습니다.

`fresh`가 아닌 항목은 사람 검토 목록으로 다룹니다. 출력에는 제목, 초록
문장, 본문, 해시 값, 영수증 경로, 검수자 아이디, 외부 서비스 주소, 로컬
절대 경로, 비밀정보가 나오지 않습니다. 초록의 진실성·완전성·유용성이나
AI가 실제로 이해했다는 사실까지 증명하는 검진은 아닙니다.
