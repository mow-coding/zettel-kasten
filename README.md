...
```text
v0.3.139 pre-release
```

Previous public baseline: v0.3.138 pre-release.
...
- read-only Tiro full-data lossless recovery planning plus approval-gated raw Tiro recovery bundle capture into WOM objets, preserving private raw bundle bytes while reporting only hashes, counts, archive-relative paths, and gap categories,
