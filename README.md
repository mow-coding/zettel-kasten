...
Previous public baseline: v0.4.45.
...
- v0.4.46 retries the Git backup's exact add only while another program briefly holds the Git index, and records writer-session coverage as complete; see the [release notes](wom-kit/docs/releases/v0.4.46.md).
- v0.4.45 gives the helper AI a twelve-rule core card, an intent-to-command table and model/reasoning guidance; see the [release notes](wom-kit/docs/releases/v0.4.45.md).
...
- v0.4.40 reopens 27 closed writers under exact approval, removes 12 commands no customer path needs, and scopes a session close to one activity; see the [release notes](wom-kit/docs/releases/v0.4.40.md).
- v0.4.39 keeps reviewed activity-cleanup plans stable on Windows, names Git backup causes, and prepares one deliverable feedback letter; see the [release notes](wom-kit/docs/releases/v0.4.39.md).
- v0.4.38 joins unpublished zet revision, authenticated session object lists, immediate offload and exact external activity cleanup into a reviewed preserve-then-finish flow; see the [release notes](wom-kit/docs/releases/v0.4.38.md).
- v0.4.37 scopes storage to the current session by default, supports explicitly delegated sessions or object lists, and preserves partial-upload effects when abandoning an overbroad plan.
- v0.4.36 answers beta letter 168 and restores the 2026-09-17 decision: a session grant opens no dialog for any operation kind (the grant itself is the only dialog); the upload writer names its cause and refuses its two silent gates before the dialog; a failure before the first checkpoint closes its claim; `operator-feedback-archive` moves delivered letters out of the archive leaving a stub; finalize re-scans only when the receipts changed; the permission preview accepts the approve request; compose creates the draft record by default.
- v0.4.35 (hotfix, beta letter 167): after the 2026-09-20 public-history rewrite `project-version-update` diagnoses a refused fetch (`fetch.rejection_kind`, `target_tag_on_remote`, `origin_main_rewritten`) and accepts a rewritten origin main only under `--affirm-origin-main-rewritten`.
- v0.4.34 answers beta letter 165: a session grant is presenter-bound and time-boxed (`presenter_token` returned once, `grant_hours`, fixed refusal codes on the result, presenter evidence in every grant claim, `session_permission_attention` in `ai-start-here`); one legacy-identifier detector warns on every new-record surface (`--allow-warnings` on `create-draft`, never under a grant); `operator-feedback-compose --approve` asks the dialog and its receipt names the claim; the revise path is announced; revision-plan warnings explain themselves.
- v0.4.33 reopens `object-storage-upload` under the exact approval contract (beta letter 164 ①③④) — see the line below; the inventory is now 60 approval-available / 59 fixed-closed.
...
- v0.4.33 reopens `object-storage-upload` under the exact approval contract (beta letter 164 ①③④): writer line first, a counts-only classification of every manifest object, `--local-bytes-only`, a create-only content-addressed PUT with a same-run full-GET proof, one manifest projection adding the `wom_uploaded` locations, execution receipts written after it; an existing remote copy is never overwritten and the remote object is never deleted. Restore and offload now cover v0.4.33 uploads too.
...
- v0.4.19 makes project update, runtime inspection, command availability, and Windows child-process behavior tell the same truth. Runtime checks distinguish `passed`, `failed`, `not_reached`, and `unavailable`; updater preparation reports privacy-safe changed dimensions instead of one opaque boolean; help, capabilities, Doctor suggestions, dry-run interpretation, and CLI dispatch share one availability decision; and noninteractive Windows children no longer open transient consoles while human approval and credential interaction remain visible. Publishing or installing the release changes no client archive. See the [v0.4.19 release notes](wom-kit/docs/releases/v0.4.19.md),
