...
```text
v0.3.185 pre-release
```

Previous public baseline: v0.3.184 pre-release.
...
- scoped `validate --since` / `validate --scope` checks with generated-index body SHA cache support and optional `--progress`, plus stage/count progress output for long `doctor --strict`, large `object-storage-adopt-existing`, and `staged-cleanup-check` runs; large adopt plan resolution now uses a per-run manifest index, explicit `--skip-existing-wom-uploaded` resume helper, resume-gap counts for matching `declared_uploaded` versus `wom_uploaded` locations, and same-provider nonmatching store/key diagnostics, doctor reuses per-run file SHA/frontmatter caches and reports deeper `mint-receipts` target-frontmatter and mint-link sub-step/cache progress, and staged cleanup can show content-free walk/verify/hash progress while remaining report-only,
