...
Previous public baseline: v0.3.222 pre-release.
...
- read-only `archive ai-start-here <archive-root> --dry-run --format markdown|json` that quickly projects runtime-context, canonical entrypoint, authority, and operational-context metadata without walking every zet or receipt; a complete archive health scan is explicit through `--full-doctor`, progress de-duplicates unchanged counts, names each work unit, and uses a fixed content-free heartbeat phase such as `file_hash` or `edge_receipt_index` while same-count private substeps are coalesced before formatter work, and both modes report their actual read boundary without calling providers or writing archive state,
