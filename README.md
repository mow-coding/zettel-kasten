...
- v0.3.275 adds read-only `archive zet-title-remap-revert-recovery-plan --dry-run` to give every complete retained revert journal one fixed privacy-safe future cleanup, forward-completion, receipt-finalization, or forensic-hold decision; it writes nothing, every action remains non-executable, and a dedicated approval-gated revert recovery executor remains later work,
