...
```text
v0.3.297 pre-release
```

Previous public baseline: v0.3.296 pre-release.
...
- closed private objet source-metadata and audience-safe label schemas plus a pure Unicode 17 normalization/projection reference module, followed by CLI-only `archive objet-source-metadata-write --dry-run|--approve` for one reviewed private observation; approval is local-NTFS Windows-only, binds exact intake/plan digests, requires private-review and all-other-writer-quiescence affirmations, and produces an append-only row plus immutable private/restricted receipt with deterministic replay/recovery; the existing `archive index` now projects only that receipt-bound authority into four disposable private tables in the same public/private transaction, while `archive index-health --dry-run` reports a closed content-free validity/freshness envelope through an opaque `mode=ro` session; SHA-256 remains identity, runtime private artifacts remain private, and no finder, private search result, new CLI/MCP surface, object-byte read, provider call, template-database change, or public-name projection is added; see [Private Objet Metadata And Safe Labels](wom-kit/docs/private-objet-metadata-safe-label.md) and [v0.3.297 release notes](wom-kit/docs/releases/v0.3.297.md),
