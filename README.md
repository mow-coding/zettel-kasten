...
Previous public baseline: v0.3.220 pre-release.
...
- read-only `archive ai-start-here <archive-root> --dry-run --format markdown|json` that projects the existing runtime-context, canonical entrypoint, and operational-context metadata into one compact first-read map for an entering AI operator, without reading zettel bodies, objet bytes, secrets, or provider APIs,
