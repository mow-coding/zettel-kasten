...
Previous public baseline: v0.3.179 pre-release.
...
- scoped `validate --since` / `validate --scope` checks with generated-index body SHA cache support and optional `--progress`, plus stage/count progress output for long `doctor --strict` and large `object-storage-adopt-existing` runs; large adopt plan resolution now uses a per-run manifest index, and doctor reuses per-run file SHA/frontmatter caches during receipt checks,
