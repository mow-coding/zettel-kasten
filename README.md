...
Previous public baseline: v0.3.300 pre-release.
...
- one integrated real-use feedback surface through Letter 112 with collision-safe records, archive-root path authority, bounded source-intake and Objet-capture batches, structured zet-objet links, reviewed occurrence-aware locators and relation judgments, GFM table plus exact-byte markup normalization/recovery/revert, reversible never-minted draft discard, archive-specific AI writing rules, project bytecode repair, and complete deterministic CI shards,
...
- read-only `archive zet-title-remap-plan --dry-run` for human-reviewed repair proposals when imported titles are identifier-shaped, followed by CLI-only approval-gated `archive zet-title-remap-write --dry-run|--approve`; v0.3.269 binds the unchanged proposal/plan/write digests plus explicit human review, preserves every complete prior canonical byte as a verified content-addressed object before replacing only the top-level YAML `title` scalar, writes a private title/body-text-free receipt last, rolls back handled failures, and retains a private journal plus common lock after a hard exit; v0.3.270 adds read-only `archive zet-title-remap-receipt-audit --dry-run` to verify all bounded title receipts, snapshots, retained journals, current participant hashes, and the common lock without echoing private evidence; v0.3.271 adds read-only `archive zet-title-remap-recovery-plan --dry-run` to map each complete retained case to one fixed privacy-safe cleanup, rollback, or forensic-hold decision; v0.3.272 adds CLI-only approval-gated `archive zet-title-remap-recover --dry-run|--approve` to execute one freshly rebound non-forensic-hold case toward verified prior bytes or verified residue cleanup while retaining evidence on interruption; v0.3.273 adds read-only `archive zet-title-remap-revert-plan --dry-run` to bind one completed receipt, exact current applied bytes, verified prior-byte snapshots, and the clean history audit into a privacy-safe compensation plan; v0.3.274 adds approval-gated `archive zet-title-remap-revert --dry-run|--approve`, exact prior-byte restoration, a separate immutable compensation receipt, durable revert interruption evidence, and archive-wide completed/interrupted revert audit; retained revert hard-exit recovery remains a later release,
- v0.3.275 adds read-only `archive zet-title-remap-revert-recovery-plan --dry-run` to give every complete retained revert journal one fixed privacy-safe cleanup, forward-completion, receipt-finalization, or forensic-hold decision; v0.3.276 adds separate approval-gated `archive zet-title-remap-revert-recover --dry-run|--approve`, which rebinds one complete plan/case/action, shares the recovery guard, continues only toward verified prior bytes, finalizes or preserves the deterministic compensation receipt, retains evidence on interruption, and never executes `manual_forensic_hold`,
...
- read-only `action_routing` in runtime-context, ai-start-here, operational-context, and canonical entrypoints tells an AI which official WOM command handles search, checked-layer objet rediscovery before any global absence claim, local version truth, inbox pipeline-shape audit, draft creation, minting, typed edges, source capture, and operating-context updates; `archive objet-rediscovery-plan --dry-run` reports all ten fixed checked/unchecked layers without echoing the private query or search rows, and never upgrades an index-only zero result into "the file does not exist"; generated AGENTS templates start with `ai-start-here`, raw grep/SQL are not authoritative WOM search, direct AI Markdown writes to `inbox/` are forbidden, `archive inbox-pipeline-audit --dry-run` reports conservative privacy-safe review signals without automatic repair, local version output is not remote release freshness proof, and persistent saved-view writing remains unimplemented; see [Objet Rediscovery Plan](wom-kit/docs/objet-rediscovery-plan.md), [AI Command-Path Routing](wom-kit/docs/ai-command-path-routing.md), and [Inbox Pipeline Audit](wom-kit/docs/inbox-pipeline-audit.md),
- closed private objet source-metadata and audience-safe label schemas plus a pure Unicode 17 normalization/projection reference module, followed by CLI-only `archive objet-source-metadata-write --dry-run|--approve` for one reviewed private observation; approval is local-NTFS Windows-only, binds exact intake/plan digests, requires private-review and all-other-writer-quiescence affirmations, and produces an append-only row plus immutable private/restricted receipt with deterministic replay/recovery; the existing `archive index` projects only that receipt-bound authority into four disposable private tables in the same public/private transaction, `archive index-health --dry-run` reports a closed content-free validity/freshness envelope through an opaque `mode=ro` session, and v0.3.298 adds CLI-only `archive find-objet` for bounded exact private alias lookup without reflecting or storing the query; SHA-256 remains identity, runtime private artifacts remain private, and no MCP, object-byte read, provider call, template-database change, or public-name projection is added; see [Private Objet Finder](wom-kit/docs/private-objet-finder.md) and [v0.3.298 release notes](wom-kit/docs/releases/v0.3.298.md),
- read-only preview layers for runtime context, profiles, source/objet intake, overview-first zet reading plus a human document view that hides raw YAML frontmatter, block headers with first-read summaries, and prompt boundaries; optional bounded `frontmatter.abstract` plus CLI/MCP `zet-catalog` enumerate every local zet with compact abstract-plus-tie reading, strict snapshot/token-budgeted zet coverage, measured item/service-result/response-envelope cost plus an opt-in envelope reserve, a required full first page followed by optional compact continuation responses that omit only repeated diagnostics, optional verified-seed connection order, and opt-in `routed_reading` explanations for each seed/tie/disconnected-component step, while separate readiness signals distinguish complete zet visitation, readable non-redacted abstracts, and unique id follow-up without requiring a generated index/map or creating persistent WOM-owned goal/loop state; non-redacted `read-zettel` supplies exact file/body hashes for the version actually read and supports opt-in Unicode character pages whose continuations are bound to the first page's complete body hash, `zet-abstract-backfill-plan` validates bounded private missing-abstract proposals, approval-gated `zet-abstract-backfill-write` applies only human-reviewed SHA-bound abstracts with a text-free receipt and whole-batch runtime rollback, receipt-bound `zet-abstract-backfill-revert` restores exact pre-backfill bytes only when no later canonical edit exists, and both batch directions publish a private text-free journal before their first canonical mutation so `zet-abstract-backfill-receipt-audit` can distinguish prepared, partial, complete-without-receipt, and divergent interrupted states; read-only `zet-abstract-backfill-recovery-plan` maps those states to evidence cleanup, apply rollback, revert forward completion/finalization, or manual hold, while approval-gated `zet-abstract-backfill-recover` binds exactly one non-forensic case to the complete plan digest, fresh review, and explicit archive quiescence before any safe-direction write or evidence cleanup,
...
- approval-gated `archive migrate --target base-link-types` that appends every base WOM-kit link type missing from a vendored `types.yml` or only repeated exact `--link-type` selections without overwriting same-id custom records; a receipt-bound partial `--revert` removes only selected records that are still base-equivalent and unused, while an archive with no local `types.yml` remains a safe no-op because it already inherits the base,
...
- read-only relation review that reserves `continues` for the next week/installment of the same course or work and activates manual-only `sequence` for a generic process's next step; `sequence` and `format_variant` cannot be auto-written through `zettel-edge-batch`, recurrence remains a coordinate rather than an edge, and reviewed non-owner actors can be registered as durable Principal targets without replacing the archive owner,
