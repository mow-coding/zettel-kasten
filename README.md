...
```text
v0.3.178 pre-release
```

Previous public baseline: v0.3.177 pre-release.
...
- scoped `validate --since` / `validate --scope` checks with generated-index body SHA cache support and optional `--progress`, plus stage/count progress output for long `doctor --strict` and large `object-storage-adopt-existing` runs,
