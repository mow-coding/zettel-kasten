...
```text
v0.3.295 pre-release
```

Previous public baseline: v0.3.294 pre-release.
...
- closed private objet source-metadata and audience-safe label schemas plus a pure Unicode 17 normalization/projection reference module; SHA-256 remains identity, public-generic output has no free-form private value, ambiguity never picks an input-order winner, and the release adds no metadata writer, index, finder, CLI, or MCP surface; see [Private Objet Metadata And Safe Labels](wom-kit/docs/private-objet-metadata-safe-label.md),
