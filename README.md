...
Previous public baseline: v0.3.281 pre-release.
...
- an explicit event-membership workflow that validates one human-selected anchor and member set, applies only digest-bound `facets.activity_group` additions through a human-approved, locked, snapshot/journal/receipt-backed writer with separately approved interruption recovery, and separately plans exact human-selected removals without writing; it infers no membership and the removal writer remains unimplemented,
