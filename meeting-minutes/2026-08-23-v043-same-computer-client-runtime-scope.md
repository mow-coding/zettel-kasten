...
- Client의 `parent_of_archive/.zettel-kasten/source`와 프로젝트 핀은 v0.4.0이었다.
- v0.4.3은 개발 worktree에서만 실행했으며 공용 도구에는 설치하지 않았다.
- v0.4.3 소스로 Client을 읽기 전용 검사한 결과는 running v0.4.3과 project
...
- GitHub Release와 wheel 검증, 공용 CLI 교체, 프로젝트 source/pin 업데이트는
  서로 다른 결과로 기록한다.
- Client 실제 업데이트는 Client 프로젝트에 결속된 exact-human workflow로만
  수행한다.
...
실제 Client 데이터, 공용 Python 도구, PATH, provider, 자격증명은 변경하지 않았다.
