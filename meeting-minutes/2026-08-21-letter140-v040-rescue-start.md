...
The work must protect the read-only Client archive, avoid disturbing unfinished Letter 138 work, and use separate worktrees plus explicit verification gates.
...
- The Client letter files remain immutable evidence and are not test fixtures.
...
2. Reproduce only with synthetic or repository fixtures; never run mutating commands against Client.
...
The CLI end-to-end synthetic fixture passed the complete sequence `dry-run -> exact plan digest -> approved writer -> v0.2 receipt -> receipt lookup`. The private label was stored only in the target zettel and did not appear in stdout or stderr. No Client path was used.
...
No Client, repository, Git, provider, or network mutation was performed by that
planning track.
