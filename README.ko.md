...
- 사람이 검토한 가져오기 제목 수정안을 위한 read-only `archive zet-title-remap-plan --dry-run`이 있습니다. v0.3.268은 최대 2,000 Unicode 글자를 받고, 줄바꿈과 기타 비정규화 공백을 나눠 알리며, 실제 일치 값을 숨긴 고정 안전 규칙 이름을 제공합니다. 일반 공개 인용 URL은 경고와 함께 허용하고 `basis: human_written` 절차를 문서화하지만, 승인된 제목 쓰기·영수증·감사·복구·되돌리기는 아직 구현하지 않았습니다.
