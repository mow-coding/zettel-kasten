...
- 완료된 전체 검진 인수인계 문서는 ERROR/WARN을 단순 개수로 버리지 않고 제한된 항목, 전체 코드별 개수, 권장 명령을 남깁니다. compact heartbeat는 보존된 과거 엣지 집계보다 현재 local-profile secret-safety의 파일·내용·프로필 처리량을 우선하며, 일반 파일 안전 순회는 확인한 디렉터리 경계를 재사용하면서 심볼릭 링크 이탈 검사는 엄격하게 유지합니다.
