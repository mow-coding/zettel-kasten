...
- runtime-context, ai-start-here, operational-context, canonical entrypoint의 읽기 전용 `action_routing`은 검색, 로컬 버전 확인, inbox 파이프라인 모양 조사, 초안 생성, 발행, 타이핑된 엣지, 출처 capture, 작업기록 갱신에 어떤 공식 WOM 명령을 써야 하는지 AI에게 알려줍니다. 새 AGENTS 템플릿은 `ai-start-here`로 시작하고, raw grep/SQL은 WOM의 권위 있는 검색이 아니며, AI가 `inbox/`에 Markdown을 직접 쓰는 행위는 금지됩니다. `archive inbox-pipeline-audit --dry-run`은 자동 수리 없이 보수적이고 비공개 안전한 검토 신호를 냅니다. 로컬 버전 출력은 원격 최신 릴리스 증거가 아니고, 영구 saved-view writer는 아직 구현되지 않았습니다. [AI Command-Path Routing](wom-kit/docs/ai-command-path-routing.md)과 [Inbox Pipeline Audit](wom-kit/docs/inbox-pipeline-audit.md)을 보세요.
...
- 선택적 [MOW Harness 호환 경계](wom-kit/docs/mow-harness-compatibility.md). `collab/`과 `.mow-harness/`를 로컬 전용으로 유지하고, 하네스 없이도 WOM을 쓸 수 있게 하며, 외부의 읽기 전용 미리보기와 별도 승인형 업데이트·활성화를 구분합니다.
