...
- read-only `archive ai-start-here <archive-root> --dry-run --format markdown|json`과 빠른 기본 CLI/MCP 인수인계 문서(`runtime-context`)는 모든 zet와 영수증을 훑지 않고 정체성, canonical entrypoint, 로컬 주권, 작업기록을 보여줍니다. 전체 검진은 `--full-doctor`를 명시할 때만 실행하며, 엣지 영수증 이력 검사는 파일명 지도 하나를 만든 뒤 SHA가 다른 zet에 속한 영수증만 읽고 별도 개수로 진행 상황을 보여줍니다. AI 스타팅 메뉴얼은 인수인계 문서가 이미 포함되었다고 표시하고 `completed_commands`와 `next_commands`를 분리하므로 AI가 같은 단계를 곧바로 반복하지 않습니다.
