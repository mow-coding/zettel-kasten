...
- v0.4.8은 중단된 exact 작업의 내구성 상태를 쉽게 설명하고 안전한 resume·revert control 하나를 자동으로 찾습니다. 또한 title·marker 복구 근거를 바로잡고, locator sidecar 읽기 범위를 제한하되 occurrence anchor는 진단 단서로만 유지하며, 기존 intake 근거에서만 capture selection을 만들고, provider 접속 없이 local object-storage 설정을 등록하며, 엄격한 duplicate pair를 정보 손실 없이 정합하고 인증된 중단 journal이나 manifest 전체를 정확히 되돌릴 수 있게 합니다. 릴리스를 설치하거나 공개하는 것만으로 클라이언트 보관함은 바뀌지 않습니다. [v0.4.8 릴리스 노트](wom-kit/docs/releases/v0.4.8.md)를 보세요.
