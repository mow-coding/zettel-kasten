...
- 표준 형식과 호환되는 `wom-archive` Agent Skill 묶음. 처음 읽는 `SKILL.md`는 97줄로 제한하고, 자주 쓰는 일을 다섯 개의 집중 참고문서로 나누며, 정확한 고급 명령이 필요할 때만 기존 전체 운영 계약을 읽습니다. 릴리스 검사는 메타데이터, 링크와 경로 안전, 모든 참고문서의 발견 가능성, 문맥 길이, 승인·비밀정보 경계를 함께 지킵니다. 이 구조는 AI가 필요한 설명을 더 잘 찾게 할 뿐이며, 사람이 검토한 로컬 WOM 기록이 정본이라는 원칙과 조용한 자동 호스트 설치를 금지하는 경계는 그대로입니다.
