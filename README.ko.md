...
- runtime-context, ai-start-here, operational-context, canonical entrypoint의 읽기 전용 `action_routing`은 검색, 로컬 버전 확인, 초안 생성, 발행, 타이핑된 엣지, 출처 capture, 작업기록 갱신에 어떤 공식 WOM 명령을 써야 하는지 AI에게 알려줍니다. 새 AGENTS 템플릿은 `ai-start-here`로 시작하고, raw grep/SQL은 WOM의 권위 있는 검색이 아니며, AI가 `inbox/`에 Markdown을 직접 쓰는 행위는 금지됩니다. 로컬 버전 출력은 원격 최신 릴리스 증거가 아니고, 영구 saved-view writer는 아직 구현되지 않았습니다. [AI Command-Path Routing](wom-kit/docs/ai-command-path-routing.md)을 보세요.
