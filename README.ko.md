...
- v0.4.12는 현재 generation-bound SQLite projection을 `zettel-objet-link`의 권한으로 사용하고, 이미 존재하는 exact link는 승인이나 쓰기 없이 결정적으로 `already_present`를 반환합니다. zet writer는 같은 generation의 delta-or-dirty lifecycle을 공유하며 index rebuild가 필요하면 정본 쓰기 전에 멈춥니다. parser 정본 inventory는 승인 가능 47개·고정 차단 67개입니다. 설치만으로 클라이언트 보관함은 바뀌지 않습니다. 업데이트 뒤 명시적 index rebuild와 health 검증 및 기술 근거 대조는 AI가 수행하며 사람에게 파일 개수나 해시 비교를 요구하지 않습니다. [v0.4.12 릴리스 노트](wom-kit/docs/releases/v0.4.12.md)를 보세요.
