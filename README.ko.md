...
- 사람이 선택한 이벤트 anchor와 명시적 canonical member 집합만 검증하는 제한·비공개 안전·읽기 전용 `activity-group-membership-plan`. O(N²) edge 증가 없이 중립적인 다대일 이벤트 소속을 표현하고 현재/제안 byte hash를 묶되, 모든 쓰기는 이후 승인 기반 릴리스로 미룹니다.
