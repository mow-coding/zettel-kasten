...
- 닫힌 비공개 objet 출처 메타데이터·안전 label schema와 순수 Unicode 17 정규화/projection reference module을 제공합니다. SHA-256은 계속 신원이고, 공개 generic 결과에는 자유 문자열 비공개 값이 없으며, 모호한 후보 중 하나를 입력 순서로 고르지 않습니다. 이번 릴리스에는 metadata writer, index, finder, 새 CLI/MCP가 없습니다. [Private Objet Metadata And Safe Labels](wom-kit/docs/private-objet-metadata-safe-label.md)을 보세요.
