...
- 사람이 선택한 이벤트 anchor와 member 집합을 먼저 검증한 뒤, 정확한 해시에 묶인 `facets.activity_group` 추가만 사람 승인·잠금·이전 상태 보존·journal·영수증을 갖춘 writer로 적용하고 별도 승인 복구까지 제공하는 workflow. 멤버십을 추론하거나 제거하지 않습니다.
