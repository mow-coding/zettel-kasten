...
- v0.3.275는 read-only `archive zet-title-remap-revert-recovery-plan --dry-run`으로 완전한 잔류 revert journal마다 미래의 증거 정리·순방향 완료·영수증 마무리·수동 보존 중 한 가지 고정 판단을 내립니다. 아무것도 쓰지 않고 모든 action은 실행 불가이며, 전용 승인형 revert 복구 실행기는 다음 릴리스 경계입니다.
