...
- v0.4.9는 새 파일의 source record부터 capture selection, 검증된 local byte 보존까지 exact 단일 파일 경로를 완성합니다. doctor 진행을 기본적으로 보이게 하고 object manifest 결과를 실행 종료 시점의 신선도에 결속하며, 추천 명령의 dry-run 가능 여부와 승인 폐쇄 여부를 구분하고, profile registry나 provider를 건드리지 않으면서 object-storage 필수 profile 안내를 바로잡습니다. 배치 writer와 doctor 성능 최적화는 별도 작업이며 설치만으로 클라이언트 보관함은 바뀌지 않습니다. [v0.4.9 릴리스 노트](wom-kit/docs/releases/v0.4.9.md)를 보세요.
