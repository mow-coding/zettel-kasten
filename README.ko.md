...
- Windows 별도 검은 입력 창과 별도 자식 프로세스를 쓰는 자격증명 수령, 인증된 수명주기, 그리고 Letter 118의 577건+43건 전체 검토 목록을 위한 승인형 Notion 복구를 제공합니다. 미리보기는 비밀값을 읽거나 provider를 호출하거나 파일을 쓰지 않고, 승인된 제한 범위만 정확한 Markdown objet와 재개 가능한 증거로 보존합니다. PAT를 명령 인자·일반 stdin·환경 변수로 받거나 워크스페이스 전체를 검색하거나 Notion에 쓰거나 canonical zet을 자동 수정하지 않습니다. [Letter 118·119 가이드](wom-kit/docs/letter118-119-credential-continuity-and-notion-page-recovery.md)를 보세요.
...
- v0.3.318은 비밀값 권한을 늘리지 않으면서 Windows 자격증명 검은 창을 이해하고 대응할 수 있게 고칩니다. `Ctrl+V`, `Shift+Insert`, Windows Terminal 기본 `Ctrl+Shift+V`, host 설정에 따라 달라지는 오른쪽 클릭을 정확히 안내하고, 완전한 비어 있지 않은 한 줄을 받으면 `입력값을 받았습니다. 검증 중입니다.`만 보여 줍니다. 프롬프트 동안 `Ctrl+C`는 무시하고 빈 Enter만 취소로 안내합니다. v0.2 결과 봉투 아래 고정 코드 다섯 개가 취소/빈 입력, 콘솔 입력 실패, provider 인증 거부, provider 신원 서비스 장애, 검토 페이지 접근 불가를 구분하며 pre-store/rollback 관계도 엄격히 검증합니다. 합성 Win32 API canary는 Unicode, echo-off, 상태 문구, Ctrl+C 생존, cleanup 순서와 복원을 확인했지만 모든 terminal host의 실제 물리 paste gesture를 증명하지는 않습니다. 인증된 자격증명은 계속 다시 묻지 않고 재사용하고 WOM은 clipboard를 프로그램으로 읽지 않습니다. [Letter 131 자격증명 콘솔 paste와 실패 단계](wom-kit/docs/letter131-credential-console-paste-and-failure-stages.md), [업그레이드 가이드](UPGRADE.ko.md), [v0.3.318 릴리스 노트](wom-kit/docs/releases/v0.3.318.md)를 보세요.
...
- 사람이 검토한 가져오기 제목 수정안을 위한 read-only `archive zet-title-remap-plan --dry-run` 뒤에 CLI 전용 승인형 `archive zet-title-remap-write --dry-run|--approve`가 이어집니다. v0.3.269 writer는 변경되지 않은 제안·계획·쓰기 digest와 명시적 사람 검토를 모두 묶고, YAML 최상위 `title` scalar만 바꾸기 전에 모든 정본 원본 바이트를 검증된 콘텐츠 주소형 object로 보존합니다. 마지막에 제목/본문 텍스트가 없는 비공개 영수증을 쓰고, 일반 오류는 원복하며, 강제 종료는 비공개 journal과 공통 lock을 남깁니다. v0.3.270은 비공개 값을 되비추지 않는 read-only `archive zet-title-remap-receipt-audit --dry-run`으로 모든 제한 범위 영수증·snapshot·잔류 journal·현재 참여 파일 hash·공통 lock을 검증합니다. v0.3.271은 read-only `archive zet-title-remap-recovery-plan --dry-run`으로 완전한 잔류 사건마다 고정된 정리·원복·수동 보존 판단을 내립니다. v0.3.272는 CLI 전용 승인형 `archive zet-title-remap-recover --dry-run|--approve`로 새로 재검증된 단일 사건을 검증된 이전 바이트 방향으로 복구하거나 정확한 잔여 증거만 정리하며, 중단 시 증거를 보존합니다. v0.3.273은 read-only `archive zet-title-remap-revert-plan --dry-run`으로 완료 영수증, 현재 적용 바이트, 검증된 이전 바이트 snapshot, 깨끗한 이력 감사를 하나의 비공개 보상 계획에 묶습니다. v0.3.274는 승인형 `archive zet-title-remap-revert --dry-run|--approve`로 정확한 이전 바이트를 복원하고, 별도 불변 보상 영수증과 강제 종료용 revert journal을 남기며, 감사기가 완료·중단 revert를 검증합니다. 잔류 revert 강제 종료 복구는 다음 릴리스 경계입니다.
- v0.3.275는 read-only `archive zet-title-remap-revert-recovery-plan --dry-run`으로 완전한 잔류 revert journal마다 증거 정리·순방향 완료·영수증 마무리·수동 보존 중 한 가지 고정 판단을 내립니다. v0.3.276은 별도 승인형 `archive zet-title-remap-revert-recover --dry-run|--approve`로 전체 계획·단일 사건·고정 action을 다시 묶고, 같은 복구 guard 아래 검증된 이전 바이트 방향으로만 진행하며, 결정론적 보상 영수증을 완성하거나 그대로 보존하고, 재중단 시 증거를 남깁니다. `manual_forensic_hold`는 계속 실행할 수 없습니다.
