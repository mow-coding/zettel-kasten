...
- v0.3.317은 두 가지 실사용 안전 경계를 바로잡습니다. 자격증명 최초 등록은 도우미 AI가 현재 작업을 설명하고 WOM이 고정 보안 문구를 제공하는 별도 Unicode Windows 검은 창에서 이루어지며, 인증된 기존 자격증명은 매번 다시 묻지 않고 재사용합니다. 내부 integration은 Notion의 bot 작업공간 ID에, 사람용 PAT는 정확한 토큰 범위 근거와 현재 페이지 확인에 묶이므로 같은 PAT를 다른 검토 페이지에서도 재사용할 수 있습니다. 예전 인증 영수증은 두 번째 입력이나 자격증명 저장소 쓰기 없이 local 범위 승계를 할 수 있고, 중복·복잡한 lifecycle은 사람 검토에서 멈춥니다. `staged-cleanup-check`는 바이트·manifest·store·receipt 증거가 모두 맞는지 끝까지 다시 검사하고, deferred 항목을 staging에 남겨 전체 삭제를 막으며, 실제 파일을 지우지 않은 채 내용 없는 저장 결과와 제한된 operation control을 제공합니다. [Letter 118·119 가이드](wom-kit/docs/letter118-119-credential-continuity-and-notion-page-recovery.md), [staged-cleanup 증거 결정](wom-kit/docs/archive-infra-decision-log-2026-08-13-v03317-letter130-staged-cleanup-evidence.md), [업그레이드 가이드](UPGRADE.ko.md), [v0.3.317 릴리스 노트](wom-kit/docs/releases/v0.3.317.md)를 보세요.
