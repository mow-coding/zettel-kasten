...
상태: v0.4.21 조건부 GitHub wheel 계약; 다시 연 writer와 승인 한 번의 반입
...
[v0.4.41 릴리스 노트](releases/v0.4.41.md)를 보세요. v0.4.19부터 v0.4.35까지의 노트는
runtime 사실성, 세션 소유 쓰기, 다시 연 writer, update 실패, fidelity 원본, 승인 모드, archive root update, 승인 창 페이지 이동, 후속 요청, 오브제 저장소 되찾기, 오브제 저장소 비우기, 편지 163 핵심, 편지 163 나머지, 편지 164 전반부, 다시 연 upload, 제시 토큰 승인 모드, 다시 써진 원격 main delta의 기록으로 남습니다.

설치된 이전 runtime에는 v0.4.36의 창 없는 승인 모드·업로드 원인 코드·편지 정리, v0.4.35의 다시 써진 원격 main 진단, v0.4.34의 제시 토큰에 묶인 승인 모드, v0.4.33의 다시 연 upload writer, v0.4.32의 편지 164 전반부, v0.4.31의 편지 163 나머지, v0.4.30의 편지 163 mint 게이트와 클레임 저장소, v0.4.29의 오브제 저장소 비우기, v0.4.28의 오브제 저장소 되찾기, v0.4.27의 후속 요청 반영, v0.4.26의 대상 자세히 보기 창 수정, v0.4.25의 archive root update 수정, v0.4.24의 세션 승인 모드, v0.4.23의 바이너리 fidelity 원본과 세션에 묶인 초안, v0.4.22의 project update 실패 사실성과 started claim 포기,
...
새 프로세스가 정확히 `archive 0.4.41`을 보고하면 그 명시적 부트스트랩으로
...
프로젝트 업데이트 뒤 일반 write 작업 전에 AI가 보관함 projection을 명시적으로
다시 만들고 건강 상태를 검증합니다.
...
개수·해시·generation·health 근거는 WOM이 검증하며 사람이 직접 세거나 digest를
비교하지 않습니다.
...
v0.4.18은 project가 다른 버전으로 넘어간 뒤에도 자기 cleanup을 끝내지 못한 완료된
원본 하나도 마무리합니다. dry-run과 approval은 `terminal_cleanup_required`를
`exact_terminal_transaction_cleanup_requires_resume` 근거와 함께 보고하고,
identifier 없는 resume은 archive에서 원래 승인 claim을 다시 인증한 뒤 그 비공개
directory만 canonical proof 하나로 정리하며 `update_completed: false`,
`past_update_success_attributed: false`가 담긴
`terminal_transaction_cleanup_completed`를 반환합니다. Windows 밖에서는 아무것도
쓰지 않고 `terminal_cleanup_platform_unsupported`를 반환합니다.
