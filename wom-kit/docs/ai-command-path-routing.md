...
| Inspect an output-supervised long command | `archive operation-control <exact-starting-root> --operation-ref op:sha256:<digest> --action status --dry-run --format json` | Available for `project-version-update`, `index`, or `index-health` with a bound output; a new approved updater may choose its private project-scoped output automatically. Use bounded `wait` or read-only `recovery-plan` after caller timeout; do not start a duplicate writer. Generic `operation-control` cancel and resume are unsupported. The separately authenticated `project-version-update --resume` route is the only updater-specific exception: pending terminal delivery reuses the exact bound output without `--output`, its immutable journal is never extended with a display event, and consumed handoff state is history rather than replay authority. Fresh updater dry-run and approval share one cleanup preflight; `project_version_update_terminal_cleanup_required` routes to identifier-free `--resume`, while `project_version_update_terminal_cleanup_outcome_unknown` means stop and preserve evidence rather than loop recovery or edit private controls. |
...
| Review and preserve one local source file | Start with `archive source-intake <archive-root> --dry-run --local-path <file> --format json`, save that private plan, then preview `source-intake-record --source-intake-plan <plan.json> --dry-run`. | The v0.4.9 exact one-file chain remains available: approve `source-intake-record`, then `objet-capture-selection --exact-existing-intake`, then `objet-capture --exact-local`, each with its fresh plan and native human decision. A preview alone grants no copy or upload authority. |
| Review and preserve 1–1,000 local source files | Prepare one strict private batch request, then run `archive source-intake-batch <archive-root> --manifest <request.json> --dry-run --format json`; approve the exact plan as the first native decision. | If intake is interrupted, keep the request unchanged and use `--resume --reviewed-by <same-actor>`; WOM discovers the one authenticated started claim without manual ids or another decision. Pass the successful content-free execution SHA-256 to `archive objet-capture-batch <archive-root> --source-intake-execution-sha256 <digest> --dry-run --format json`, then make the separate second native decision. WOM derives the request path and verifies the authenticated upstream chain. Partial or uncertain capture requires a fresh dry-run and new approval, never replay or same-claim resume. Provider, upload, linking, draft, mint, and unscoped legacy routes remain closed. |
...
- v0.4.10 keeps the machine `action_routing` schema at v0.13 and replaces the
  historical batch-capture route text with the authenticated two-decision
  workflow. Intake can auto-discover and resume one unchanged authenticated
  started claim without another native decision. Capture always begins from
  the completed intake execution SHA-256; an interrupted or uncertain capture
  requires a fresh dry-run and a new approval, never same-claim or per-item
  replay.
