...
Previous checkpoint: Status: v0.4.35 rewritten-origin hotfix, letter-167, and session-owned writes truth

Previous checkpoint: Status: v0.4.34 presenter-bound grants, letter-165, and session-owned writes truth
...
Previous checkpoint: Status: v0.4.31 letter-163 remainder, letter-163 mint gate and claim store, and session-owned writes truth

Previous checkpoint: Status: v0.4.30 letter-163 mint gate and claim store, object-storage offload, and session-owned writes truth

Previous checkpoint: Status: v0.4.29 object-storage offload, object-storage restore, and session-owned writes truth
...
Previous checkpoint: Status: v0.4.26 target-details dialog hotfix, archive-root update hotfix, and session-owned writes truth

Previous checkpoint: Status: v0.4.25 archive-root update hotfix, session permission modes, and session-owned writes truth

Previous checkpoint: Status: v0.4.24 session permission modes, session-bound drafts, and session-owned writes truth

Previous checkpoint: Status: v0.4.23 binary fidelity sources, session-bound drafts, and session-owned writes truth

Previous checkpoint: Status: v0.4.22 project-update failure truth, started-claim abandon, and session-owned writes truth
...
version. Dry-run, approval, and resume classify that directory identically as
`terminal_original_exact`; resume holds the project terminal guard,
re-authenticates the original approval claim from the archive as the cleanup
authority, and cleans only that private directory into one canonical proof.
The result `terminal_transaction_cleanup_completed` attributes no past success
and grants no fresh approval authority; a new update still needs a new preview
and one exact approval. When the live pin still matches the transaction
post-image, the v0.4.16 replay contract runs first and the same cleanup is its
fallback after the fixed candidate-missing gate. A masked
`exact_human_approval_state_unknown` may now carry one allowlisted inner
`cause_code` in the redacted failure artifact. The command inventory is
unchanged.

v0.4.17 gave fresh `project-version-update` preview and approval the same bounded,
...
When an updater bootstrap is needed and the exact public v0.4.36 GitHub Release
...
Require exactly `archive 0.4.41` from a new process. This does not update the
