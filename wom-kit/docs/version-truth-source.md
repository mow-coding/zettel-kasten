...
Status: v0.4.24 session permission modes, session-bound drafts, and session-owned writes
...
Current checkpoint: Status: v0.4.24 session permission modes with installed-wheel hash evidence
...
The v0.4.24 URL is a conditional release-artifact contract. Use it only after
...
Require exactly `archive 0.4.24` from a new process. The external CPython 3.12
