...
Current release candidate: v0.4.46 Git backup retries only a briefly held index, and writer-session coverage is complete; [triage decision](archive-infra-decision-log-2026-09-24-closed-writer-triage.md). The public stable baseline remains v0.4.45 until publication.

Status: v0.4.46 git index retry and writer-session coverage audit

Previous checkpoint: Status: v0.4.45 helper-AI core rules card, intent table and model guidance

Previous checkpoint: Status: v0.4.44 superseded IMAP chain removed, notion-recover revived, grants until released

Previous checkpoint: Status: v0.4.43 project update approval hotfix (letter 20260925-173)
...
Previous checkpoint: Status: v0.4.40 27 writers reopened under exact approval, 12 commands removed, activity-scoped close

Previous checkpoint: Status: v0.4.39 stable cleanup plans, named Git backup causes, and one deliverable feedback letter
...
Previous checkpoint: Status: v0.4.36 no-dialog grants, letter-168, and session-owned writes

Previous checkpoint: Status: v0.4.35 rewritten-origin hotfix, letter-167, and session-owned writes
...
Previous checkpoint: Status: v0.4.32 letter-164 first half, letter-163 remainder, and session-owned writes

Previous checkpoint: Status: v0.4.31 letter-163 remainder, letter-163 mint gate and claim store, and session-owned writes

Previous checkpoint: Status: v0.4.30 letter-163 mint gate and claim store, object-storage offload, and session-owned writes

Previous checkpoint: Status: v0.4.29 object-storage offload, object-storage restore, and session-owned writes

Previous checkpoint: Status: v0.4.28 object-storage restore, client follow-ups, and session-owned writes

Previous checkpoint: Status: v0.4.27 client follow-ups, target-details dialog hotfix, and session-owned writes

Previous checkpoint: Status: v0.4.26 target-details dialog hotfix, archive-root update hotfix, and session-owned writes

Previous checkpoint: Status: v0.4.25 archive-root update hotfix, session permission modes, and session-owned writes

Previous checkpoint: Status: v0.4.24 session permission modes, session-bound drafts, and session-owned writes

Previous checkpoint: Status: v0.4.23 binary fidelity sources, session-bound drafts, and session-owned writes

Previous checkpoint: Status: v0.4.22 project-update failure truth, started-claim abandon, and session-owned writes

Previous checkpoint: Status: v0.4.21 reopened writers, one-approval intake chain, and session-owned writes

Current checkpoint: Status: v0.4.46 git index retry and coverage audit candidate; installed-wheel and public evidence pending

Previous checkpoint: Status: v0.4.45 helper-AI guidance with installed-wheel hash evidence

Previous checkpoint: Status: v0.4.44 retire/revive/grant with installed-wheel hash evidence

Previous checkpoint: Status: v0.4.43 update approval hotfix with installed-wheel hash evidence
...
Previous checkpoint: Status: v0.4.40 closed-writer triage with installed-wheel hash evidence

Previous checkpoint: Status: v0.4.39 letter 173 fixes with installed-wheel hash evidence
...
Previous checkpoint: Status: v0.4.30 letter-163 mint gate and claim store with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.29 object-storage offload with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.28 object-storage restore with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.27 client follow-ups with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.26 target-details dialog hotfix with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.25 archive-root update hotfix with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.24 session permission modes with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.23 binary fidelity sources and session-bound drafts with installed-wheel hash evidence
Previous checkpoint: Status: v0.4.22 project-update failure truth and started-claim abandon with installed-wheel hash evidence
...
Previous checkpoint: Status: v0.4.17 cleanup-preflight parity, terminal-history recovery, and installed-wheel hash truth
...
The v0.4.46 URL is a conditional release-artifact contract. Use it only after
...
Require exactly `archive 0.4.46` from a new process. The external CPython 3.12
...
and the [v0.4.19 release note](releases/v0.4.19.md).

v0.4.19 makes the runtime, updater, command inventory, and actual dispatch
describe the same observed state. Runtime checks distinguish `passed`,
`failed`, `not_reached`, and `unavailable`; an unreadable value is not
silently rewritten as an observed mismatch or absence. The approved updater
revalidates Git, refs, pins, policy, supply, launcher, materialization, and the
prepared payload as separate privacy-safe dimensions. Help, capabilities,
Doctor suggestions, dry-run interpretation, and dispatch share one
`CapabilityAvailability` result, so a fixed-closed approval is rejected before
private work starts. Noninteractive Windows child processes use the common
no-console policy while native approval and credential interaction remain
visible.
...
identifier-free `--resume` classify it as `terminal_original_exact`; resume
re-authenticates the original approval claim from the archive as the cleanup
authority, cleans only that private directory into one canonical proof, and
returns `terminal_transaction_cleanup_completed` without attributing past
success. `marker.json` is an identity anchor; the verified checkpoint journal
is the completion truth.
...
For a v0.4.19 project, the `project_runtime` object is also canonical evidence.
