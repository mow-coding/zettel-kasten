...
Status: v0.4.21 conditional GitHub wheel contract; reopened writers and one-approval intake
...
The v0.4.21 URL below is a conditional contract, not proof that an artifact is
public. Use it only after the matching GitHub Release exists and lists the
verified wheel. See the [v0.4.21 release note](releases/v0.4.21.md) for the
separate source and release-evidence boundary; the v0.4.19 and v0.4.20 notes
remain the record of the runtime-truth and session-owned-write deltas.

An installed older runtime may not contain v0.4.21's reopened writers and
one-approval intake chain, v0.4.20's session-owned writes, or v0.4.19's
four-state runtime observations and Windows no-console child-process policy.
Updating repository files alone does not replace an installed wheel. After the
verified v0.4.21 asset exists, install that exact wheel and start a new process.
...
After the new process reports exactly `archive 0.4.21`, use that explicit
