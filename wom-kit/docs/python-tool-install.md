...
The v0.4.41 URL below is a conditional contract, not proof that an artifact is
public. Use it only after the matching GitHub Release exists and lists the
verified wheel. See the [v0.4.41 release note](releases/v0.4.41.md) for the
...
letter-163 remainder, letter-164 first-half, reopened-upload,
presenter-bound-grant and rewritten-origin deltas.

An installed older runtime may not contain v0.4.36's no-dialog grants,
upload cause codes and letter archival, v0.4.35's rewritten-origin
...
remainder, v0.4.30's letter-163 mint gate
and claim store, v0.4.29's object-storage
offload, v0.4.28's object-storage
restore, v0.4.27's client follow-ups,
v0.4.26's target-details
dialog fix, v0.4.25's archive-root update
fix, v0.4.24's session permission
modes, v0.4.23's binary fidelity sources
and session-bound drafts, v0.4.22's project-update failure
truth and started-claim abandon, v0.4.21's reopened writers and one-approval
intake chain, v0.4.20's session-owned writes, or v0.4.19's four-state runtime
observations and Windows no-console child-process policy. Updating repository
files alone does not replace an installed wheel. After the verified v0.4.22
asset exists, install that exact wheel and start a new process. A project
reserved by a failed v0.4.21 update is released with the v0.4.22 bootstrap
`archive` (`--resume --abandon-started-approval`, then `--resume`), because
the older project launcher does not know the flag.
...
After the new process reports exactly `archive 0.4.41`, use that explicit
...
& "$womToolRoot\Scripts\python.exe" -m pip install "https://github.com/mow-coding/zettel-kasten/releases/download/v0.4.41/wom_kit-0.4.41-py3-none-any.whl"
...
v0.4.17 gives fresh dry-run and approval the same read-only terminal-cleanup
preflight. `project_version_update_terminal_cleanup_required` means exact
control history must first converge through identifier-free
`project-version-update --resume`; it does not require a target, transaction
ref, approval id, file count, or digest. Recovery may compact only an exact
WOM-produced preapproval-abort history, enters no project-domain writer, and
retains canonical proof history. Run a new preview after recovery and request
one fresh approval only when that preview is ready.

v0.4.18 also finishes one completed original that outlived its own cleanup
after the project moved to another version. Dry-run and approval report
`terminal_cleanup_required` with the basis
`exact_terminal_transaction_cleanup_requires_resume`; identifier-free resume
re-authenticates the original approval claim from the archive, cleans only that
private directory into one canonical proof, and returns
`terminal_transaction_cleanup_completed` with `update_completed: false` and
`past_update_success_attributed: false`. Off Windows it returns
`terminal_cleanup_platform_unsupported` with zero writes.

`project_version_update_terminal_cleanup_outcome_unknown` still means stop.
Do not loop resume or edit a lock, pin, transaction directory, tombstone, or
proof. Preserve the structured privacy-safe result for development review.
