...
The v0.4.30 URL below is a conditional contract, not proof that an artifact is
public. Use it only after the matching GitHub Release exists and lists the
verified wheel. See the [v0.4.30 release note](releases/v0.4.30.md) for the
separate source and release-evidence boundary; the v0.4.19 through v0.4.29
...
object-storage-restore and object-storage-offload deltas.

An installed older runtime may not contain v0.4.30's letter-163 mint gate
...
After the new process reports exactly `archive 0.4.30`, use that explicit
...
& "$womToolRoot\Scripts\python.exe" -m pip install "https://github.com/mow-coding/zettel-kasten/releases/download/v0.4.30/wom_kit-0.4.30-py3-none-any.whl"
