...
file. The caller supplies that file for ordinary observation; a v0.4.19
...
that means the caller must pass `--output`. A v0.4.19 approved or resumed
...
v0.4.19 also keeps observation failure separate from operation failure. A
runtime, lock, candidate, or source check that cannot be read reliably is
`unavailable`; it is not converted into a confirmed mismatch, absence, or
cleanup success. Doctor and updater progress still start immediately and emit
content-free heartbeat state while long checks run.
...
For a successful v0.4.16 through v0.4.19 project update, the terminal journal is immutable
