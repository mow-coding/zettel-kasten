...
| Prehashed external objet ledger | `read-only preview` | none | CLI `archive prehashed-objet-ledger --dry-run` and MCP `prehashed_objet_ledger_preview` preview already-hashed external content-addressed ledgers such as Notion source-export retrieval ledgers. They count sha256/byte-size rows without echoing row values, reading blob bytes, registering manifests, or claiming that `objet-capture` can skip byte verification today. |
