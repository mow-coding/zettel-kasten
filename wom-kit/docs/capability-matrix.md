...
Status: v0.3.25 credential adapter readiness planning checkpoint
Date: 2026-06-15
Version: v0.3.25, release candidate
...
| Credential access approval plan | `read-only preview` | none | CLI `archive credential-access-approval-plan --decision approve_once|deny|needs_review --dry-run` and MCP `credential_access_approval_plan` preview the future human-reviewed approval receipt required before a broker can use a credential. They write no approval receipts, grant no live approval, open no password manager, open no browser password store, open no OS keyring, read no environment variables, read no plaintext secret note, call no providers, echo no exact credential ref values, and include no secret values in the receipt preview. |
