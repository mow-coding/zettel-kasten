...
| Prehashed external objet ledger | `read-only preview` | none | CLI `archive prehashed-objet-ledger --dry-run` previews already-hashed external content-addressed ledgers such as Notion source-export retrieval ledgers. It counts sha256/byte-size rows without echoing row values, reading blob bytes, registering manifests, or claiming that `objet-capture` can skip byte verification today. |
