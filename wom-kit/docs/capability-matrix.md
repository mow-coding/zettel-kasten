...
| Provider setup status | `read-only preview` | none | `archive provider-status --dry-run` checks setup-managed GitHub/object-storage bindings against local provider setup receipts. It does not call providers, verify live accounts, upload, sync, push, or write files. |
