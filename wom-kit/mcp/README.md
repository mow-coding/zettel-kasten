...
zet_catalog
  Enumerate every selected local zet node's available compact frontmatter abstract state and connections in deterministic read-only pages. Use `projection: reading`, `coverage_mode: strict`, and optional host-sized token pages. Every page measures items, compact service result, and response envelope; optional `response_envelope_reserve_tokens` partitions an explicit total budget. Keep cursor zero at `response_profile: full`, then optionally use `continuation` on later strict pages to omit repeated scope diagnostics while retaining items, readiness, snapshot, token, and chain evidence. Verified-seed order remains exhaustive, and opt-in `routed_reading` adds per-item seed/tie/component reasons at a higher token cost. Follow continuation to node-coverage readiness, then inspect abstract-reading and unique-id follow-up readiness. It ranks nothing, persists no map/state, reads no zet bodies, and requires no generated index.
...
notion_objet_link_plan
  Plan Notion provider locator to manifested objet candidates for one zettel while locators still exist. This is read-only and never echoes provider URLs, body text, frontmatter values, page titles, absolute paths, object bytes, or writes files.
...
notion_objet_link_rewrite_plan
  Validate one reviewed Notion locator/object pair before any approval-gated body rewrite or embed edge write. This is read-only and never echoes provider URLs, body text, page titles, absolute paths, object bytes, or writes files.
