...
AGENTS_ROUTING_BLOCK = "\n".join(
    (
        ...
        (
            ...
            "`operator-feedback-compose` (since v0.4.34 the approve opens the "
            "exact approval dialog with a person:/human: reviewer and its "
            "receipt names the claim; `--create-draft-record` also creates "
            "the draft record so `--intent revise --expected-body-sha256` "
            "works next), verify it with "
