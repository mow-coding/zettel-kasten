...
Managed runtimes retain source-checked compilation for the two large modules.
Uncached compilation can hold the GIL longer than a heartbeat interval, so the
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            cache_finder = startup_cache.activate()
        except startup_cache.StartupCacheError:
            print("startup_cache_integrity_mismatch: repair the installed runtime before continuing.", file=sys.stderr)
            return 3
        from . import archive_cli

        return int(archive_cli.main(raw_argv))
    finally:
        if cache_finder is not None and cache_finder in sys.meta_path:
            sys.meta_path.remove(cache_finder)
