...
CURRENT_RELEASE_PATH = KIT / "docs" / "releases" / "v0.4.43.md"
CURRENT_PACKAGED_RELEASE_PATH = RESOURCE_ROOT / "release-notes" / "v0.4.43.md"
...
class V0408AndCurrentReleaseDocsTests(unittest.TestCase):
    def test_current_install_bootstrap_uses_one_exact_release_version(self) -> None:
        ...
        for surface_name, surface in (
            ...
            with self.subTest(surface=surface_name, forbidden="fixed bootstrap"):
                self.assertNotIn(
                    '$womBootstrapRoot = Join-Path $env:LOCALAPPDATA "WOM\\bootstrap-v0443"',
                    surface,
                )
        self.assertIn("exactly `archive 0.4.43`", install)
        self.assertIn("정확히 `archive 0.4.43`", install_ko)
    ...
    def test_current_release_and_resources_are_packaged_exactly(self) -> None:
        ...
        self.assertEqual(release_names, ["v0.4.43.md"])
