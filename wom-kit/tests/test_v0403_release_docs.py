...
CURRENT_RELEASE_PATH = KIT / "docs" / "releases" / "v0.4.10.md"
CURRENT_PACKAGED_RELEASE_PATH = RESOURCE_ROOT / "release-notes" / "v0.4.10.md"
...
class V0408AndCurrentReleaseDocsTests(unittest.TestCase):
    def test_current_install_bootstrap_uses_one_exact_release_version(self) -> None:
        ...
        for required in (
            ...
            "wom_kit-0.4.10-py3-none-any.whl",
        ):
            with self.subTest(required=required):
                self.assertIn(required, combined)
        self.assertIn("exactly `archive 0.4.10`", install)
        self.assertIn("정확히 `archive 0.4.10`", install_ko)
    ...
    def test_version_sources_and_wheel_contract_are_synchronized(self) -> None:
        self.assertEqual(__version__, "0.4.10")
        self.assertIn(
            'version = "0.4.10"',
        ...
        self.assertIn('version: "0.4.10"', citation)
    ...
    def test_current_release_and_resources_are_packaged_exactly(self) -> None:
        ...
        self.assertEqual(release_names, ["v0.4.10.md"])
        ...
        self.assertIn("release-notes/v0.4.10.md", packaged_paths)
    ...
    def test_v0410_release_defines_current_scope_and_privacy_boundaries(self) -> None:
        release = CURRENT_RELEASE_PATH.read_text(encoding="utf-8")
        normalized = " ".join(release.split())
        for required in (
            "source-intake-batch",
            "1–1,000",
            "authenticated approval claim",
            "fresh exact dry-run",
