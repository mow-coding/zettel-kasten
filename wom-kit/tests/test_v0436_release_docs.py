...
class V0436ReleaseDocsTests(unittest.TestCase):
    ...
    def test_release_surfaces_are_documented(self) -> None:
        ...
        self.assertIn("63 approval-available, 59 fixed-closed", " ".join(contract.split()))
        ...
        self.assertIn("approval_available:                   63", capabilities)
        matrix = (KIT / "docs" / "capability-matrix.md").read_text(encoding="utf-8")
        self.assertIn("| Operator feedback archival (v0.4.36) |", matrix)
        self.assertIn("63 approval-available, 59 fixed-closed", " ".join(matrix.split()))
    ...
    def test_coverage_manifest_matches_release_claim(self) -> None:
        ...
        self.assertEqual(routed, 9)  # v0.4.36: five pending rows and the archival row route through the environment grant
