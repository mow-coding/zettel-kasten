...
class V0436ReleaseDocsTests(unittest.TestCase):
    ...
    def test_supply_lock_and_policy_are_exact(self) -> None:
        ...
        self.assertEqual(previous.replace(b'"target_tag": "v0.4.35"', b'"target_tag": "v0.4.46"'), current)
    ...
    def test_release_surfaces_are_documented(self) -> None:
        ...
        self.assertIn("103 approval-available, 6 fixed-closed", " ".join(contract.split()))
        ...
        self.assertIn("approval_available:                   103", capabilities)
        matrix = (KIT / "docs" / "capability-matrix.md").read_text(encoding="utf-8")
        self.assertIn("| Operator feedback archival (v0.4.36) |", matrix)
        self.assertIn("103 approval-available, 6 fixed-closed", " ".join(matrix.split()))
