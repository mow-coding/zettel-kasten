...
class CapabilityMatrixDocsTests(unittest.TestCase):
    ...
    def test_connection_import_plan_maps_notion_connection_evidence_without_writes(self) -> None:
        ...
        for phrase in (
            ...
            "`material`, `derived`",
            "`view_query`",
            "recommended edge types are not yet allowed link types",
        ...
        for phrase in (
            ...
            with self.subTest(phrase=phrase):
                self.assertTrue(phrase in public_map_text or phrase in public_map_ko_text or phrase in release_text)
    ...
    def test_ai_response_concept_guide_explains_hash_location_and_layer_boundaries(self) -> None:
        ...
        for phrase in (
            ...
            "`documented-only`",
            "adds no command, MCP tool, provider call, object upload",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, matrix_text)

        for phrase in (
            "beginner-facing AI response concept guide",
            "sha256 object identity vs location",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, readme_text)
