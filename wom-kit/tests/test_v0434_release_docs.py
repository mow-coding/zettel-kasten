...
class V0434ReleaseDocsTests(unittest.TestCase):
    ...
    def test_supply_lock_and_policy_are_exact(self) -> None:
        ...
        self.assertEqual(previous.replace(b'"target_tag": "v0.4.33"', b'"target_tag": "v0.4.34"'), current)
    ...
    def test_current_docs_use_v0434_status_without_erasing_v0432_history(self) -> None:
        expected = {
            KIT / "docs" / "agent-operator-capabilities.md":
    ...
    def test_release_surfaces_are_documented(self) -> None:
        ...
        for phrase in ("presenter-bound and time-boxed", "`presenter_token`", "work_session_presenter_mismatch",
                       "`session_presenter`", "work_session_second_presenter_observed", "60 approval-available, 59 fixed-closed"):
        ...
        self.assertEqual(coverage["paths"]["operator-feedback-compose"]["status"], "pending")
        self.assertEqual(coverage["paths"]["operator-feedback-compose"]["target"], "v0.4.35")
        for row in ("object-storage-restore", "object-storage-offload", "exact-approval-claim-finalize", "object-storage-upload"):
            ...
            self.assertEqual(coverage["paths"][row]["target"], "v0.4.35")
    ...
    def test_coverage_manifest_matches_release_claim(self) -> None:
        ...
        self.assertEqual(len(statuses), 60)  # v0.4.33: object-storage-upload row; v0.4.34 adds none
        self.assertEqual(statuses.count("session_integrated") - routed, 6)
        self.assertEqual(routed, 1)
        self.assertEqual(statuses.count("pending"), 34)  # v0.4.34: operator-feedback-compose
