...
class ArchiveCliTests(unittest.TestCase):
    ...
    def test_credential_ref_plan_is_read_only_and_redacts_raw_secret_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(result["ok"])
            self.assertTrue(result["dry_run"])
            self.assertEqual(result["lifecycle_action"], "credential_ref_plan")
            self.assertEqual(result["credential_store"], "keyring")
            self.assertEqual(result["credential_kind"], "openai_api_key")
            self.assertEqual(result["purpose"], "model_api_access")
            self.assertFalse(result["current_capability"]["os_keyring_read_implemented"])
            ...
            self.assertFalse(result["closed_actions"]["openai_api_called"])
    ...
    def test_source_intake_can_attach_project_intake_receipt_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = self.copy_fake_archive(Path(tmp) / "archive")
            object_id = "sha256:acc6e73fb84988ecb538dfc0ceb883b88694e469a05172a5aeb0cce8902ce136"
            ...
            self.assertIn("scope.single_project", context["checklist_coverage"]["answered_checklist_ids"])
            self.assertTrue(any("session evidence only" in action for action in result["next_safe_actions"]))
            self.assertNotIn("One project only", output)
            self.assertNotIn('"answer"', output)

    def test_source_intake_blocks_invalid_project_intake_receipt_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = self.copy_fake_archive(Path(tmp) / "archive")
            object_id = "sha256:acc6e73fb84988ecb538dfc0ceb883b88694e469a05172a5aeb0cce8902ce136"
            decisions_path = Path(tmp) / "decisions.json"
    ...
    def test_project_intake_record_answer_dry_run_starts_session_without_echoing_answer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = Path(tmp) / "archive"
            shutil.copytree(KIT_ROOT / "examples" / "fake-life-archive", archive_root)
            answer_path = Path(tmp) / "answer.json"
            answer_text = "Keep this as one project bundle."
            answer_note = "Human reviewed the scope."
            ...
            result = json.loads(output)
            self.assertTrue(result["ok"])
            self.assertEqual(result["action"], "archive_project_intake_record_answer")
            self.assertEqual(result["session_id"], "alpha-project-20260613")
            self.assertEqual(result["previous_answer_count"], 0)
            self.assertEqual(result["new_answer_checklist_id"], "scope.single_project")
            self.assertEqual(result["expected_next_checklist_id"], "scope.single_project")
            self.assertEqual(result["answer_count"], 1)
            self.assertEqual(result["would_change"], [result["proposed_receipt_path"]])
            self.assertFalse(result["privacy_guards"]["answer_values_echoed"])
            self.assertFalse(result["privacy_guards"]["previous_decision_values_echoed"])
            self.assertFalse(result["privacy_guards"]["writes"])
            self.assertNotIn(answer_text, output)
            self.assertNotIn(answer_note, output)

    def test_project_intake_record_answer_approve_appends_to_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = Path(tmp) / "archive"
            shutil.copytree(KIT_ROOT / "examples" / "fake-life-archive", archive_root)
            first_answer = Path(tmp) / "first-answer.json"
            first_text = "One project only."
            ...
            self.assertEqual(first_code, 0, first_output)
            first_receipt = json.loads(first_output)["receipt_path"]

            second_answer = Path(tmp) / "second-answer.json"
            second_text = "Continue from the reviewed staging folder."
            second_answer.write_text(
                json.dumps(
                    {
                        "schema": "wom-kit/project-intake-answer/v0.1",
                        "checklist_id": "staging.location",
                        "answer": second_text,
                        "notes": "No restage needed.",
                    }
                ),
                encoding="utf-8",
            )
            ...
            result = json.loads(output)
            self.assertTrue(result["ok"])
            self.assertEqual(result["previous_receipt_path"], first_receipt)
            self.assertEqual(result["previous_answer_count"], 1)
            self.assertEqual(result["new_answer_checklist_id"], "staging.location")
            self.assertEqual(result["answer_count"], 2)
            self.assertNotEqual(result["receipt_path"], first_receipt)
            receipt_path = archive_root / result["receipt_path"]
            receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
            self.assertEqual(receipt["schema"], "wom-kit/project-intake-decisions-receipt/v0.1")
            self.assertEqual(receipt["action"], "archive_project_intake_decisions")
            self.assertEqual(receipt["source_answer_schema"], "wom-kit/project-intake-answer/v0.1")
            self.assertEqual(receipt["answer_count"], 2)
            self.assertEqual([item["checklist_id"] for item in receipt["decisions"]], ["scope.single_project", "staging.location"])
            self.assertEqual(receipt["decisions"][0]["answer"], first_text)
            self.assertEqual(receipt["decisions"][1]["answer"], second_text)
            self.assertFalse(receipt["closed_actions"]["objet_capture_run"])
            self.assertNotIn(first_text, output)
            self.assertNotIn(second_text, output)
            ...
            self.assertEqual(status_code, 0, status_output)
            status = json.loads(status_output)
            self.assertEqual(status["checklist_coverage"]["answered_count"], 2)
            self.assertIn("privacy.sensitive_items", status["checklist_coverage"]["missing_checklist_ids"])
            self.assertNotIn(first_text, status_output)
            self.assertNotIn(second_text, status_output)
    ...
    def test_project_intake_decision_template_starts_from_staged_folder_without_writes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = Path(tmp) / "archive"
            shutil.copytree(KIT_ROOT / "examples" / "fake-life-archive", archive_root)
            staged = Path(tmp) / "archive-objets" / "intake" / "alpha-project"
            ...
            result = json.loads(output)
            self.assertTrue(result["ok"])
            self.assertEqual(result["action"], "archive_project_intake_decision_template")
            self.assertEqual(result["state"], "needs_first_review")
            self.assertEqual(result["next_question"]["checklist_id"], "scope.single_project")
            template = result["decision_record_template"]
            self.assertEqual(template["schema"], "wom-kit/project-intake-decisions/v0.1")
            self.assertEqual(template["session_id"], "alpha-project-20260613")
            self.assertEqual(template["staged_folder_ref"], "intake:alpha-project")
            self.assertEqual(template["decisions"][0]["checklist_id"], "scope.single_project")
            self.assertIsNone(template["decisions"][0]["answer"])
            self.assertIsNone(template["decisions"][0]["notes"])
            self.assertFalse(result["template_policy"]["answer_values_included"])
            self.assertFalse(result["privacy_guards"]["file_bodies_read"])
            self.assertEqual(result["would_change"], [])
            self.assertNotIn("private-file-name.md", output)
            self.assertNotIn("SUPER_SECRET_BODY", output)

    def test_project_intake_decision_template_continues_from_receipt_without_echoing_previous_answers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            archive_root = Path(tmp) / "archive"
            shutil.copytree(KIT_ROOT / "examples" / "fake-life-archive", archive_root)
            decisions_path = Path(tmp) / "decisions.json"
            ...
            self.assertTrue(result["ok"])
            self.assertEqual(result["state"], "needs_more_review")
            self.assertEqual(result["source"]["existing_answered_checklist_ids"], ["scope.single_project"])
            self.assertFalse(result["source"]["previous_decision_values_included"])
            self.assertEqual(result["decision_record_template"]["session_id"], "alpha-project-20260613")
            self.assertEqual(result["decision_record_template"]["decisions"][0]["checklist_id"], "staging.location")
            self.assertIsNone(result["decision_record_template"]["decisions"][0]["answer"])
            self.assertFalse(result["template_policy"]["previous_answer_values_included"])
            self.assertNotIn("One project only", output)
    ...
    def test_project_intake_item_plan_previews_one_selected_file_without_writes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("source-intake-record", result["command_guidance"]["source_intake_record_approve"])
            self.assertIn("objet-capture-selection", result["command_guidance"]["objet_capture_selection_dry_run"])
            self.assertIn("<archive-relative-staged-file>", result["command_guidance"]["objet_capture_selection_dry_run"])
            self.assertTrue(result["command_guidance"]["selection_manifest_command_available"])
