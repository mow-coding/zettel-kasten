...
class V0419ReleaseDocsTests(unittest.TestCase):
    ...
    def test_supply_lock_and_policy_are_exact(self) -> None:
        ...
        current_lock = (
            KIT / "project-runtime-supply-lock-v0.4.32.json"
        ).read_bytes()
        ...
        self.assertEqual(
            policy["supply_lock"],
            "wom-kit/project-runtime-supply-lock-v0.4.32.json",
        )
        ...
        for path in BOOTSTRAP_DOCUMENTS:
            ...
            with self.subTest(path=path):
                ...
                self.assertIn(
                    ...
                    '"WOM\\bootstrap-v0432-$womBootstrapNonce"',
                    document,
                )
                self.assertRegex(
                    document,
