...
- A human-owned rough draft may be incomplete, but an AI-created draft must enter through the stricter metadata-complete route above. Before `mint-zet`, `mint-zettel`, or `promote`, require one human-reviewed explicit `frontmatter.abstract`; compatibility fields do not authorize canonical publication.
...
- Start long `index` and `index-health` work with a fresh `--output`; a new approved or unbound-resume `project-version-update` may choose its private project-scoped output automatically. Preserve the early `operation_ref`. After caller timeout, use read-only `operation-control` status, bounded wait, or recovery-plan with the exact starting root; never start a duplicate writer. Generic `operation-control` cancel/resume, MCP control, daemon, queue, background launch, force kill, and lock deletion are unsupported. The only updater-specific exception is authenticated, identifier-free `project-version-update --resume`. Pending terminal delivery must reuse its exact bound output without a new `--output`; identical display may repeat and consumed state is history. Fresh dry-run and approval share one cleanup preflight. `project_version_update_terminal_cleanup_required` routes exact control history to identifier-free resume; WOM may compact only exact preapproval-abort history into canonical proof history without a project-domain write or fresh approval authority. Proof-only needs a new preview and approval. `project_version_update_terminal_cleanup_outcome_unknown` means stop and preserve evidence; never edit locks, pins, transactions, tombstones, or proofs.
