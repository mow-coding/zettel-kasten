...
- Objet storage setup can be planned locally, but bucket creation, upload, sync, copy, hashing, source intake apply/capture, and source import remain outside WOM-kit v0.2.50.
- Profile wallet readiness can be previewed locally, but private key custody, wallet creation, real signing, blockchain integration, and payment/economic layers remain outside WOM-kit v0.2.50.
...
- Public release link hygiene checks are local validation only; GitHub Release editing and external URL network fetching remain outside WOM-kit v0.2.50.
- Korean product-language terms are public explanation guidance only; CLI commands, JSON fields, schema fields, filenames, Python identifiers, and package names remain English in WOM-kit v0.2.50.
- Prompt boundary checks and prompt-boundary draft metadata are heuristic guardrails only; complete prompt-injection prevention, LLM classification, provider scanning, OCR/import apply, and full-auto safety guarantees remain outside WOM-kit v0.2.50.
