...
- Objet storage setup can be planned locally, but bucket creation, upload, sync, copy, hashing, source intake apply/capture, and source import remain outside WOM-kit v0.2.37.
- Profile wallet readiness can be previewed locally, but private key custody, wallet creation, real signing, blockchain integration, and payment/economic layers remain outside WOM-kit v0.2.37.
- Foreign block intake, trust preview, attestation packet preview, quarantine plan, quarantine review index, quarantine decision preview, quarantine decision review index, and decision outcome plan are inspect-only; CLI quarantine write records an isolated untrusted case and receipt, and CLI quarantine decision write records only an operator-reviewed local decision and receipt. Foreign block import, trust, drafting, minting, attesting, anchoring, applying, quarantine decision acceptance, decision outcome acceptance, transport, and automatic acceptance remain outside WOM-kit v0.2.37.
- Prompt boundary checks and prompt-boundary draft metadata are heuristic guardrails only; complete prompt-injection prevention, LLM classification, provider scanning, OCR/import apply, and full-auto safety guarantees remain outside WOM-kit v0.2.37.
