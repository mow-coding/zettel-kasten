...
- Objet storage setup can be planned locally, but bucket creation, upload, sync, copy, hashing, source intake apply/capture, and source import remain outside WOM-kit v0.2.32.
- Profile wallet readiness can be previewed locally, but private key custody, wallet creation, real signing, blockchain integration, and payment/economic layers remain outside WOM-kit v0.2.32.
- Foreign block intake, trust preview, attestation packet preview, and quarantine plan are inspect-only; CLI quarantine write only records an isolated untrusted case and receipt. Foreign block import, trust, drafting, minting, attesting, anchoring, applying, transport, and automatic acceptance remain outside WOM-kit v0.2.32.
- Prompt boundary checks and prompt-boundary draft metadata are heuristic guardrails only; complete prompt-injection prevention, LLM classification, provider scanning, OCR/import apply, and full-auto safety guarantees remain outside WOM-kit v0.2.32.
