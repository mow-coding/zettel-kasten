...
The exact v0.4.41 GitHub Release, when present, uses the self-contained wheel
...
Run the version check in a new process and require exactly `archive 0.4.41`.
...
`origin_main_rewritten`) and accepts a rewritten origin main only under
`--affirm-origin-main-rewritten` (tag ref never forced; before/after ids
reported; the affirmation is bound into the approval).

v0.4.34 answers beta letter 165: presenter-bound, time-boxed session grants
(`presenter_token` once, `grant_hours`, `session_permission_refused` codes,
`session_presenter` in every grant claim, `session_permission_attention` in
`ai-start-here`), the legacy-identifier warning on create-draft / mint /
revision-plan / objet-link / intake (`--allow-warnings`; never under a grant),
`operator-feedback-compose --approve` under the exact approval dialog with a
v0.2 receipt naming the claim, the announced revise path
(`--create-draft-record`) and revision-plan warning explanations.
...
unavailable update snapshot names the failed Git probe and its fixed kind;
session start, backup evidence and work-session create/claim carry a
numbers-only Git backup attention block; `set-permission-mode --dry-run`
previews a grant without a dialog.

v0.4.31 finishes beta letter 163: `create-draft` names the options that
identify an AI runtime and marks null replay values; the mint dry-run
warns about discarded or unknown edge targets and explains the two
body-wording warnings; a `project-version-update` failure always records
a cause family and a blocked dry-run describes the existing transaction;
edge, revert-edge and intake-chain dry-runs announce the index fact.
...
the row marked `offloaded`, the remote object is never deleted and
`object-storage-restore` reverses it (OB-02; Windows-only apply).
...
refused permission grant names the refused position and the grantable
kinds, intake plans accept a UTF-8 byte-order mark, and the discard
previews expose `plan_sha256` at the top level.

v0.4.26 fixes the native approval dialog's target-details page on Windows
11: a late navigation confirmation keeps the page inert instead of
cancelling the dialog as a native failure.

v0.4.25 fixes the project update started from the archive root: recorded
`parent_of_archive/...` locations are resolved onto the project root by every
consumer, so the post-approval guard passes and `--resume` reopens the
transaction, and a failure raised before any result names its fixed cause
code and journal stage.

v0.4.24 adds per-work-session permission modes (`manual`, `limited`,
`allow_all`): one exact human decision on the claimed session lets the
operation kinds it names run without a native dialog, while every such write
still publishes its own one-use claim that records the permission mechanism;
project updates, providers, the session lifecycle, repairs, overrides and
credential writes always ask.

v0.4.23 lets a binary original be the fidelity source of a summary or
derivative draft on a `bytes` comparison basis, binds `create-draft` to a
claimed work session (the session scope is frozen into the reviewed plan and
the receipt) and revalidates the carried recovery rows.
...
failure from checks that were not reached or unavailable, updater preparation
reports privacy-safe dimension states instead of one unexplained boolean, and
fixed-closed approval flags are rejected before a handler or private target
read. Noninteractive Windows children use one no-console policy while native
...
history returns `project_version_update_terminal_cleanup_required` and routes
to identifier-free `--resume`; recovery compacts only exact plan-bound private
control evidence into canonical proof history without entering the project-
domain writer or changing source, runtime, pin, or archive content. Known
cleanup gates retain fixed privacy-safe reason codes and actionable next steps.
Partial, changed, ambiguous, mixed, or unsafe residue remains fail-closed, and
the person is never asked to count artifacts, compare hashes, or select private
transaction identifiers. Publishing or installing the wheel changes no client
archive or project pin; a client separately chooses recovery and one reviewed
project update.
...
v0.4.6 reopens only two operation-specific R2 modes. Emergency preservation
binds and independently rehashes content-addressed remote bytes without calling
...
v0.4.14 makes local locator recovery preserve exact reviewed references instead
of restoring complete historical bodies over them. It classifies proven and
partial resolution in one private exact-operation ledger, auto-discovers the
fixed receipt inventory, and keeps weak evidence in review. The same release
adds hash-bound local decision clues and a pure complete-document Markdown
display projection; unsafe clues are omitted and incomplete pages remain
canonical source. People choose run or cancel while WOM verifies identities,
counts, bytes, lineage, checkpoints, and drift. Installing this wheel does not
read or change a client archive.
...
`already_present` without approval or a durable write. Link planning and apply
bind the generation, target row, unique identities, manifest descriptor, and
stable file evidence instead of repeatedly resolving the full tree. zet writers
share a same-generation dirty-intent and exact delta-or-dirty lifecycle: a
missing or stale index fails before approval and canonical writes, while an
ambiguous partial mutation stays dirty and reports that rebuild is required.
The parser-derived current inventory is 47 approval-available, 67 fixed-closed,
and 201 not-exposed paths. Installing the wheel alone changes no client archive.
After update, the AI runs the explicit index rebuild and health check and
verifies counts, hashes, and generation evidence; the person does not count
...
only the paired source index, marker repair changes only the marker token, and
locator sidecars use a bounded stable reader while occurrence anchors remain
diagnostic-only. Existing intake evidence can mint one exact capture selection,
local object-storage setup can be registered without a provider call, and
strict duplicate pairs can be reconciled without losing either definition and
reverted from exact completed or authenticated interrupted evidence. A pending
revert resumes with `--revert --resume --reviewed-by <same-reviewer>`, reusing
the authenticated claim without another native dialog or another manifest
write. Installing the wheel alone changes no client archive.
...
the parent mints a fresh expiring one-invocation credential capability. The
isolated worker validates it and durably creates an archive-key-HMAC claim
before the first native secret read; any existing capability-id leaf blocks
replay.

Every provider attempt reauthenticates that claim and the exact
receipt/lifecycle authority, then enforces fixed read-only endpoint, selected
scope, registered-capability, and request-budget bounds. The HMAC claim stores
id/digest, request/plan, budgets, status, and count; the recovery receipt stores
only reference schema/id/digest; the parent returns a separate validated use
summary. A fully verified local replay creates no claim, reads no credential,
and calls no provider. This adds no command, popup, password manager, browser
autofill, or AI-visible secret. See the
[`v0.3.320 release note`](docs/releases/v0.3.320.md),
[`Credential Capability Contract`](docs/credential-capability-contract.md),
and historical [`Letter 132 guide`](docs/letter132-credential-console-keyboard-readiness-and-causal-evidence.md).
...
updated, and a self-consistent project mirror that differs from the running
import. Under explicit local-path disclosure, the last case can return an
exact one-invocation project-scoped bridge argv only when
`runtime_alignment.integrity.verified` is true after local real-path,
...
wrapper blob, and every synchronized resource listed in the manifest, executes the wrapper from
...
purged, `wom-kit/src` is not added to `sys.path`, and an exact-OID finder loads
only `wom_kit`, so post-gate top-level dependency shadows cannot execute from
the project tree. Those checks use no network and read no origin URL value.
The bridge does not replace the global Python tool,
infer installer provenance, or install the runtime Agent Skill. Shareable
version output exposes only exact stable labels; malformed local
pin/source/pyproject values and non-release Git tag names become null
fail-closed evidence instead of echoed payloads. The edge
safety and exact wheel-resource gates from v0.3.290 and
v0.3.289 remain in force. The v0.3.288 content-free error boundary also
remains: caller values do not enter error `message` or `data`.
...
v0.3.300 closes the remaining public implementation work from beta Letters
098-111 in one integrated release: collision-safe feedback ids, archive-root
path authority, bounded batch capture, provider-neutral locators, reviewed
relation judgments, active manual `sequence`, registered third-party
Principal targets, selected/revertible base-type adoption, crash-recoverable
markup normalization, derived-bytecode repair, and complete deterministic CI
sharding. See
...
v0.3.308 closes the Letter 115 reference, locator, and table gaps. Paired file
references now use one full-fragment digest; self-closing page mentions and
unknown-audio placeholders can use reviewed edge or manifested-objet bindings;
and a strict safe table-cell subset preserves visible inline content while
unsupported shapes remain fail closed. The new dedupe-only locator deactivate
plan/apply pair requires both the target and keeper, changes only the target to
...
`faithful_summary`, or `sanitized_derivative` mode, an intended audience, and
the exact dry-run body and source-fidelity plan hashes before attributed human
approval writes the draft and private receipt. Personal `private_self`
...
docs/zet-title-remap-plan.md
docs/zet-title-remap-write.md
docs/zet-title-remap-receipt-audit.md
docs/zet-title-remap-recovery-plan.md
...
abstract-freshness --dry-run
  Compare current abstract/body hash pairs with retained human-review evidence. It scans bounded frontmatter first with at most eight workers, opens complete zet bytes only for valid explicit-abstract targets, then opens only deterministic publication receipts for safe target ids plus bounded revision/backfill receipt directories; unsafe target ids use a conservative fallback. It reports exact frontmatter/body read counts, creates no persistent cache, and `--progress` labels the work `stage=1/2` and `stage=2/2`.
...
create-draft
  Preview or create a draft zettel in inbox/. Human-owned rough drafts keep the compatible path. Every new AI-assisted/generated route requires an abstract, stable facet, manifested source object, source-fidelity mode and audience, then an exact --approve replay with --draft-approved-by, --expected-body-sha256, and --expected-source-fidelity-plan-sha256. Declared AI provenance cannot use the human route. The private receipt is create-only; the public result does not expose source authority. This does not mint or share.

mint-zet --dry-run
  Check whether a draft zet can be minted, re-verify any source-fidelity receipt and raw body region, and preview canonical path, mint receipt, draft snapshot, and safe current fidelity-plan digest without writing.

mint-zet --approve --reviewed-by
  Mint an inbox draft zet into canonical private archive memory and write a receipt plus draft snapshot. A fidelity-bound draft also requires the exact current --expected-source-fidelity-plan-sha256; an older AI draft requires an attributed legacy_source_fidelity_reviewed affirmation.
