...
zet-catalog --dry-run
  Enumerate every local zet's compact frontmatter abstract and complete edge projection in deterministic pages. `--projection reading --coverage-mode strict` gives compact checksum-chained coverage. Optional `--order seeded_connection_walk --start-zettel-id <verified-id>` reads the verified seed neighborhood first through incoming/outgoing edge passages, then still includes every disconnected component. Only `archive_wide_coverage_claim_ready` is contiguous archive-wide evidence. It ranks nothing, writes no map/state, reads no body, and requires no generated index.
