...
zet-catalog --dry-run
  Enumerate every local zet node's available compact frontmatter abstract state and complete edge projection in deterministic pages. `--projection reading --coverage-mode strict` gives compact checksum-chained node coverage. Optional `--order seeded_connection_walk --start-zettel-id <verified-id>` reads the verified seed neighborhood first through incoming/outgoing edge passages, then still includes every disconnected component. `archive_wide_coverage_claim_ready` proves contiguous node visitation; separate `archive_wide_abstract_reading_claim_ready` and `archive_wide_followup_resolution_ready` fields report complete first-read availability and unique id follow-up safety. It ranks nothing, writes no map/state, reads no body, and requires no generated index.
