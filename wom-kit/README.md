...
zet-catalog --dry-run
  Enumerate every local zet node's available compact frontmatter abstract state and complete edge projection in deterministic pages. `--projection reading --coverage-mode strict` gives compact checksum-chained node coverage. Optional `--order seeded_connection_walk --start-zettel-id <verified-id>` reads the verified seed neighborhood first and every disconnected component afterward; opt-in `--projection routed_reading` adds each item's seed/tie/component order reason at a higher token cost. Separate signals report node coverage, first-read availability, and unique-id follow-up, while strict chain hashes distinguish duplicate-id file entries without returning paths. It ranks nothing, writes no map/state, reads no body, and requires no generated index.
